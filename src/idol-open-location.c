...
struct IdolOpenLocationPrivate
{
	GtkWidget *uri_container;
	GtkEntry *uri_entry;
};

#define IDOL_OPEN_LOCATION_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE ((obj), IDOL_TYPE_OPEN_LOCATION, IdolOpenLocationPrivate))

G_DEFINE_TYPE (IdolOpenLocation, idol_open_location, GTK_TYPE_DIALOG)

/* GtkBuilder callbacks */
G_MODULE_EXPORT void uri_entry_changed_cb (GtkEditable *entry, GtkDialog *dialog);

static void
idol_open_location_class_init (IdolOpenLocationClass *klass)
{
	g_type_class_add_private (klass, sizeof (IdolOpenLocationPrivate));
}

static void
idol_open_location_init (IdolOpenLocation *self)
{
	GtkBuilder *builder;

	self->priv = IDOL_OPEN_LOCATION_GET_PRIVATE (self);
	builder = idol_interface_load ("uri.ui", FALSE, NULL, self);
...
static gboolean
idol_open_location_match (GtkEntryCompletion *completion, const gchar *key, GtkTreeIter *iter, gpointer user_data)
{
...
static gint
idol_compare_recent_stream_items (GtkRecentInfo *a, GtkRecentInfo *b)
{
...
char *
idol_open_location_get_uri (IdolOpenLocation *open_location)
{
	char *uri;

	g_return_val_if_fail (IDOL_IS_OPEN_LOCATION (open_location), NULL);
...
static char *
idol_open_location_set_from_clipboard (IdolOpenLocation *open_location)
{
...
idol_open_location_new (void)
{
	...
	open_location = IDOL_OPEN_LOCATION (g_object_new (IDOL_TYPE_OPEN_LOCATION, NULL));
	...
	clipboard_location = idol_open_location_set_from_clipboard (open_location);
	...
	/* Add items in Idol's GtkRecentManager to the URI GtkEntry's GtkEntryCompletion */
	...
	if (recent_items != NULL)
	{
		...
		streams_recent_items = g_list_sort (streams_recent_items, (GCompareFunc) idol_compare_recent_stream_items);
	...
	gtk_entry_completion_set_match_func (completion, (GtkEntryCompletionMatchFunc) idol_open_location_match, model, NULL);
