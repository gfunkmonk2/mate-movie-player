...
#include "idol.h"
#include "idol-cmml-parser.h"
#include "idol-chapters-utils.h"
#include "idol-edit-chapter.h"

#define IDOL_TYPE_CHAPTERS_PLUGIN		(idol_chapters_plugin_get_type ())
#define IDOL_CHAPTERS_PLUGIN(o)		(G_TYPE_CHECK_INSTANCE_CAST ((o), IDOL_TYPE_CHAPTERS_PLUGIN, IdolChaptersPlugin))
#define IDOL_CHAPTERS_PLUGIN_CLASS(k)		(G_TYPE_CHECK_CLASS_CAST((k), IDOL_TYPE_CHAPTERS_PLUGIN, IdolChaptersPluginClass))
#define IDOL_IS_CHAPTERS_PLUGIN(o)		(G_TYPE_CHECK_INSTANCE_TYPE ((o), IDOL_TYPE_CHAPTERS_PLUGIN))
#define IDOL_IS_CHAPTERS_PLUGIN_CLASS(k)	(G_TYPE_CHECK_CLASS_TYPE ((k), IDOL_TYPE_CHAPTERS_PLUGIN))
#define IDOL_CHAPTERS_PLUGIN_GET_CLASS(o)	(G_TYPE_INSTANCE_GET_CLASS ((o), IDOL_TYPE_CHAPTERS_PLUGIN, IdolChaptersPluginClass))

#define IDOL_CHAPTERS_PLUGIN_GET_PRIVATE(obj)	(G_TYPE_INSTANCE_GET_PRIVATE ((obj), IDOL_TYPE_CHAPTERS_PLUGIN, IdolChaptersPluginPrivate))
...
} IdolChaptersPluginPrivate;

typedef struct {
	IdolPlugin			 parent;
	IdolObject			*idol;
	IdolEditChapter		*edit_chapter;
	IdolChaptersPluginPrivate	*priv;
} IdolChaptersPlugin;

typedef struct {
	IdolPluginClass		parent_class;
} IdolChaptersPluginClass;
...
GType idol_chapters_plugin_get_type (void) G_GNUC_CONST;
...
static void idol_chapters_plugin_finalize (GObject *object);
static void idol_file_opened_async_cb (IdolObject *idol, const gchar *uri, IdolChaptersPlugin *plugin);
static void idol_file_opened_result_cb (gpointer data, gpointer user_data);
static void idol_file_closed_cb (IdolObject *idol, IdolChaptersPlugin *plugin);
static void add_chapter_to_the_list (gpointer data, gpointer user_data);
static void add_chapter_to_the_list_new (IdolChaptersPlugin *plugin, const gchar *title, gint64 time);
static gboolean check_available_time (IdolChaptersPlugin *plugin, gint64 time);
static GdkPixbuf * get_chapter_pixbuf (GdkPixbuf *src);
static void chapter_edit_dialog_response_cb (GtkDialog *dialog, gint response, IdolChaptersPlugin *plugin);
...
static void chapter_selection_changed_cb (GtkTreeSelection *tree_selection, IdolChaptersPlugin *plugin);
static void show_chapter_edit_dialog (IdolChaptersPlugin *plugin);
static void save_chapters_result_cb (gpointer data, gpointer user_data);
static GList * get_chapters_list (IdolChaptersPlugin *plugin);
static gboolean show_popup_menu (IdolChaptersPlugin *plugin, GdkEventButton *event);
static void mateconf_autoload_changed_cb (MateConfClient *mateconf, guint cnx_id, MateConfEntry *entry, IdolChaptersPlugin	*plugin);
static void load_chapters_from_file (const gchar *uri, gboolean from_dialog, IdolChaptersPlugin *plugin);
static void set_no_data_visible (gboolean visible, gboolean show_buttons, IdolChaptersPlugin *plugin);

/* GtkBuilder callbacks */
void add_button_clicked_cb (GtkButton *button, IdolChaptersPlugin *plugin);
void remove_button_clicked_cb (GtkButton *button, IdolChaptersPlugin *plugin);
void save_button_clicked_cb (GtkButton *button, IdolChaptersPlugin *plugin);
void goto_button_clicked_cb (GtkButton *button, IdolChaptersPlugin *plugin);
void tree_view_row_activated_cb (GtkTreeView *tree_view, GtkTreePath *path, GtkTreeViewColumn *column, IdolChaptersPlugin *plugin);
gboolean tree_view_button_press_cb (GtkTreeView *tree_view, GdkEventButton *event, IdolChaptersPlugin *plugin);
gboolean tree_view_key_press_cb (GtkTreeView *tree_view, GdkEventKey *event, IdolChaptersPlugin *plugin);
gboolean tree_view_popup_menu_cb (GtkTreeView *tree_view, IdolChaptersPlugin *plugin);
void popup_remove_action_cb (GtkAction *action, IdolChaptersPlugin *plugin);
void popup_goto_action_cb (GtkAction *action, IdolChaptersPlugin *plugin);
void load_button_clicked_cb (GtkButton *button, IdolChaptersPlugin *plugin);
void continue_button_clicked_cb (GtkButton *button, IdolChaptersPlugin *plugin);

IDOL_PLUGIN_REGISTER (IdolChaptersPlugin, idol_chapters_plugin)

static void
idol_chapters_plugin_class_init (IdolChaptersPluginClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);
	IdolPluginClass *plugin_class = IDOL_PLUGIN_CLASS (klass);

	g_type_class_add_private (klass, sizeof (IdolChaptersPluginPrivate));

	object_class->finalize = idol_chapters_plugin_finalize;
...
static void
idol_chapters_plugin_init (IdolChaptersPlugin *plugin)
{
	plugin->priv = IDOL_CHAPTERS_PLUGIN_GET_PRIVATE (plugin);
}

static void
idol_chapters_plugin_finalize (GObject *object)
{
	G_OBJECT_CLASS (idol_chapters_plugin_parent_class)->finalize (object);
}
			 ...
			 gpointer	user_data)
{
	...
	g_return_if_fail (IDOL_IS_CHAPTERS_PLUGIN (user_data));

	plugin = IDOL_CHAPTERS_PLUGIN (user_data);
	tree = plugin->priv->tree;
	store = GTK_TREE_STORE (gtk_tree_view_get_model (GTK_TREE_VIEW (plugin->priv->tree)));
	clip = ((IdolCmmlClip *) data);

	/* prepare tooltip data */
	start = idol_cmml_convert_msecs_to_str (clip->time_start);
...
static void
add_chapter_to_the_list_new (IdolChaptersPlugin	*plugin,
			     const gchar		*title,
			     gint64			time)
{
	GdkPixbuf	*pixbuf;
	...
	g_return_if_fail (IDOL_IS_CHAPTERS_PLUGIN (plugin));
	...
	/* prepare tooltip data */
	start = idol_cmml_convert_msecs_to_str (time);
...
static gboolean
check_available_time (IdolChaptersPlugin	*plugin,
		      gint64			time)
{
	GtkTreeModel	*store;
	GtkTreeIter	iter;
	...
	g_return_val_if_fail (IDOL_IS_CHAPTERS_PLUGIN (plugin), FALSE);
...
static void
idol_file_opened_result_cb (gpointer	data,
			     gpointer	user_data)
{
	IdolCmmlAsyncData	*adata;
	IdolChaptersPlugin	*plugin;

	g_return_if_fail (data != NULL);

	adata = (IdolCmmlAsyncData *) data;
	plugin = IDOL_CHAPTERS_PLUGIN (adata->user_data);
	...
	if (G_UNLIKELY (!adata->successful)) {
		if (G_UNLIKELY (g_cancellable_is_cancelled (adata->cancellable))) {
			...
			g_free (adata->file);
			g_free (adata->error);
			g_list_foreach (adata->list, (GFunc) idol_cmml_clip_free, NULL);
			g_list_free (adata->list);
			g_free (adata);
			return;
		} else
			idol_action_error (_("Error while reading file with chapters"),
					    adata->error, plugin->idol);
	}
...
static void
idol_file_opened_async_cb (IdolObject			*idol,
			    const gchar			*uri,
			    IdolChaptersPlugin		*plugin)
{
	gchar	*cmml_file;
	...
	cmml_file = idol_change_file_extension (uri, "cmml");
...
static void
idol_file_closed_cb (IdolObject		*idol,
		      IdolChaptersPlugin	*plugin)
{
	GtkTreeStore	*store;

	g_return_if_fail (IDOL_IS_OBJECT (idol) && IDOL_IS_CHAPTERS_PLUGIN (plugin));
				 ...
				 IdolChaptersPlugin	*plugin)
{
	...
	g_return_if_fail (IDOL_IS_EDIT_CHAPTER (dialog));
	g_return_if_fail (IDOL_IS_CHAPTERS_PLUGIN (plugin));
	...
	if (response != GTK_RESPONSE_OK) {
		...
		if (plugin->priv->was_played)
			idol_action_play (plugin->idol);
		return;
	}

	gtk_widget_hide (GTK_WIDGET (dialog));

	title = idol_edit_chapter_get_title (IDOL_EDIT_CHAPTER (dialog));
		     ...
		     gpointer			user_data)
{
	...
	time_str = idol_cmml_convert_msecs_to_str (time);
...
static void
show_chapter_edit_dialog (IdolChaptersPlugin	*plugin)
{
	...
	main_window = idol_get_main_window (plugin->idol);
	plugin->priv->was_played = idol_is_playing (plugin->idol);
	idol_action_pause (plugin->idol);

	/* adding a new one, check if it's time available */
	g_object_get (G_OBJECT (plugin->idol), "current-time", &time, NULL);
	if (G_UNLIKELY (!check_available_time (plugin, time))) {
		idol_interface_error_blocking (_("Chapter with the same time already exists"),
	...
	/* capture frame */
	bvw = BACON_VIDEO_WIDGET (idol_get_video_widget (plugin->idol));
	...
	plugin->edit_chapter = IDOL_EDIT_CHAPTER (idol_edit_chapter_new ());
			 ...
			 IdolChaptersPlugin	*plugin)
{
	...
	if (G_UNLIKELY (idol_cmml_read_file_async (data) < 0)) {
			 ...
			 gpointer	user_data)
{
	...
	if (G_UNLIKELY (!adata->successful && !g_cancellable_is_cancelled (adata->cancellable))) {
		idol_action_error (_("Error while writing file with chapters"),
				    adata->error, plugin->idol);
		gtk_widget_set_sensitive (plugin->priv->save_button, TRUE);
	}
...
static GList *
get_chapters_list (IdolChaptersPlugin	*plugin)
{
	GList		*list = NULL;
	IdolCmmlClip	*clip;
	GtkTreeModel	*store;
	...
	g_return_val_if_fail (IDOL_IS_CHAPTERS_PLUGIN (plugin), NULL);
	...
	while (valid) {
		gtk_tree_model_get (store, &iter,
				    ...
				    CHAPTERS_PIXBUF_COLUMN, &pixbuf,
				    -1);
		clip = idol_cmml_clip_new (title, NULL, time, pixbuf);
...
static gboolean
show_popup_menu (IdolChaptersPlugin	*plugin,
		 GdkEventButton		*event)
{
	...
	menu = gtk_ui_manager_get_widget (plugin->priv->ui_manager, "/idol-chapters-popup");
			...
			IdolChaptersPlugin	*plugin)
{
	...
	if (G_UNLIKELY (idol_cmml_write_file_async (data) < 0)) {
		idol_action_error (_("Error occurred while saving chapters"),
				    _("Please check you rights and free space"), plugin->idol);
		g_free (data);
			    ...
			    IdolChaptersPlugin		*plugin)
{
	...
	seekable = idol_is_seekable (plugin->idol);
	if (!seekable) {
	...
	idol_action_seek_time (plugin->idol, time, TRUE);
}
			...
			IdolChaptersPlugin	*plugin)
{
	...
	mrl = idol_get_current_mrl (plugin->idol);
	main_window = idol_get_main_window (plugin->idol);
	       ...
	       GError **error)
{
	...
	cplugin = IDOL_CHAPTERS_PLUGIN (plugin);
	main_window = idol_get_main_window (idol);

	builder = idol_plugin_load_interface (plugin, "chapters-list.ui", TRUE,
					       main_window, cplugin);
	g_object_unref (main_window);
	...
	cplugin->priv->ui_manager = GTK_UI_MANAGER (gtk_builder_get_object (builder, "idol-chapters-ui-manager"));
	...
	cplugin->idol = g_object_ref (idol);
	/* for read operation */
	...
	idol_add_sidebar_page (idol, "chapters", _("Chapters"), main_box);
	...
	g_signal_connect (G_OBJECT (idol),
			  "file-opened",
			  G_CALLBACK (idol_file_opened_async_cb),
			  plugin);
	g_signal_connect (G_OBJECT (idol),
			  "file-closed",
			  G_CALLBACK (idol_file_closed_cb),
			  plugin);
	...
	mrl = idol_get_current_mrl (cplugin->idol);
	if (mrl != NULL)
		idol_file_opened_async_cb (cplugin->idol, mrl, cplugin);
		 ...
		 IdolObject *idol)
{
	...
	g_object_unref (cplugin->idol);
	g_free (cplugin->priv->cmml_mrl);

	idol_remove_sidebar_page (idol, "chapters");
}
