...
#define IDOL_CMML_PREAMBLE 	"<!DOCTYPE cmml SYSTEM \"cmml.dtd\">\n"

typedef void (*IdolCmmlCallback) (IdolCmmlClip *, gpointer user_data);
...
typedef struct {
	...
	IdolCmmlStatus		status;
	IdolCmmlClip		*clip;
	IdolCmmlCallback	callback;
	gpointer		user_data;
} IdolCmmlContext;

static IdolCmmlContext * idol_cmml_context_new (void);
static void idol_cmml_context_free (IdolCmmlContext *context);
static void idol_cmml_context_set_callback (IdolCmmlContext *context, IdolCmmlCallback cb, gpointer user_data);
static int idol_cmml_compare_clips (gconstpointer pointer_a, gconstpointer pointer_b);
static IdolCmmlClip * idol_cmml_clip_new_from_attrs (const xmlChar **attrs);
static void idol_cmml_clip_insert_img_attr (IdolCmmlClip *clip, const xmlChar **attrs);
static gdouble idol_cmml_parse_smpte (const gchar *str, gdouble framerate);
static gdouble idol_cmml_parse_npt (const gchar *str);
static gdouble idol_cmml_parse_time_str (const gchar *str);
static void idol_cmml_parse_start (IdolCmmlContext *context, const xmlChar *tag, const xmlChar **attrs);
static void idol_cmml_parse_end (IdolCmmlContext *context, const xmlChar *tag);
static void idol_cmml_parse_xml_node (IdolCmmlContext	*context);
static void idol_cmml_read_clip_cb (IdolCmmlClip *clip, gpointer user_data);

static IdolCmmlContext *
idol_cmml_context_new (void)
{
	IdolCmmlContext *ctx = g_new0 (IdolCmmlContext, 1);

	ctx->status = IDOL_CMML_NONE;
	return ctx;
}

static void
idol_cmml_context_free (IdolCmmlContext *context)
{
	g_free (context);
}

static void
idol_cmml_context_set_callback (IdolCmmlContext	*context,
				 IdolCmmlCallback	cb,
				 gpointer		user_data)
{
...
static IdolCmmlClip *
idol_cmml_clip_new_from_attrs (const xmlChar **attrs)
{
	gint		i = 0;
	gint64		start = -1;
	gchar		*title = NULL;
	...
	while (attrs[i] != NULL) {
		if (g_strcmp0 ((const gchar *) attrs[i], "title") == 0)
			...
			start = (gint64) (idol_cmml_parse_time_str ((const gchar *) attrs[i + 1]) * 1000);
		i += 2;
	}

	return idol_cmml_clip_new (title, NULL, start, NULL);
}

static void
idol_cmml_clip_insert_img_attr (IdolCmmlClip	*clip,
				 const xmlChar	**attrs)
{
	gint		i = 0;
	GdkPixdata	*pixdata;
...
static gdouble
idol_cmml_parse_smpte (const gchar	*str,
			gdouble		framerate)
{
	gint		h = 0, m = 0, s = 0;
	gfloat		frames;
...
static gdouble
idol_cmml_parse_npt (const gchar *str)
{
	gint		h, m;
	gfloat		s;
...
static gdouble
idol_cmml_parse_time_str (const gchar *str)
{
	gchar	timespec[16];
	...
	if (sscanf (str, "npt:%16s", timespec) == 1)
		return idol_cmml_parse_npt (str + 4);

	if (sscanf (str, "smpte-24:%16s", timespec) == 1)
		return idol_cmml_parse_smpte (str + 9, 24.0);

	if (sscanf (str, "smpte-24-drop:%16s", timespec) == 1)
		return idol_cmml_parse_smpte (str + 14, 23.976);

	if (sscanf (str, "smpte-25:%16s", timespec) == 1)
		return idol_cmml_parse_smpte (str + 9, 25.0);

	if (sscanf (str, "smpte-30:%16s", timespec) == 1)
		return idol_cmml_parse_smpte (str + 9, 30.0);

	if (sscanf (str, "smpte-30-drop:%16s", timespec) == 1)
		return idol_cmml_parse_smpte (str + 14, 29.97);

	if (sscanf (str, "smpte-50:%16s", timespec) == 1)
		return idol_cmml_parse_smpte (str + 9, 50.0);

	if (sscanf (str, "smpte-60:%16s", timespec) == 1)
		return idol_cmml_parse_smpte (str + 9, 60);

	if (sscanf (str, "smpte-60-drop:%16s", timespec) == 1)
		return idol_cmml_parse_smpte (str + 14, 59.94);

	/* default is npt */
	return idol_cmml_parse_npt (str);
}

static void
idol_cmml_parse_start (IdolCmmlContext	*context,
			const xmlChar		*tag,
			const xmlChar		**attrs)
{
	g_return_if_fail (context != NULL);
	...
	if (g_strcmp0 ((const gchar *) tag, "cmml") == 0) {
		if (G_UNLIKELY (context->status != IDOL_CMML_NONE))
			return;
		...
		context->status = IDOL_CMML_CMML;
	} else if (g_strcmp0 ((const gchar *) tag, "head") == 0) {
		if (G_UNLIKELY (context->status != IDOL_CMML_CMML))
			return;
		...
		context->status = IDOL_CMML_HEAD;
	} else if (g_strcmp0 ((const gchar *) tag, "clip") == 0) {
		if (G_UNLIKELY (context->status != IDOL_CMML_CMML))
			return;

		context->clip = idol_cmml_clip_new_from_attrs (attrs);
		...
		if (G_UNLIKELY (xmlTextReaderIsEmptyElement (context->reader))) {
			...
			context->status = IDOL_CMML_CMML;
			idol_cmml_clip_free (context->clip);
			context->clip = NULL;
		} else
			context->status = IDOL_CMML_CLIP;
	} else if (g_strcmp0 ((const gchar *) tag, "img") == 0) {
		if (G_UNLIKELY (context->status != IDOL_CMML_CLIP))
			return;

		idol_cmml_clip_insert_img_attr (context->clip, attrs);
	}
}

static void
idol_cmml_parse_end (IdolCmmlContext	*context,
		      const xmlChar	*tag)
{
	g_return_if_fail (context != NULL);
	g_return_if_fail (tag != NULL);
	...
	if (g_strcmp0 ((const gchar *) tag, "cmml") == 0) {
		...
		context->status = IDOL_CMML_NONE;
	} else if (g_strcmp0 ((const gchar *) tag, "head") == 0) {
		if (G_UNLIKELY (context->status != IDOL_CMML_HEAD))
			return;
...
static void
idol_cmml_parse_xml_node (IdolCmmlContext *context)
{
	xmlChar		*tag;
	xmlChar		**attrs = NULL;
	gint		j, i = 0;
	...
	if (xmlTextReaderNodeType (context->reader) == XML_READER_TYPE_ELEMENT) {
		...
		idol_cmml_parse_start (context, tag, (const xmlChar **) attrs);
		/* free resources */
		for (j = i - 1; j >= 0; j -= 1)
			xmlFree (attrs[j]);
		g_free (attrs);
	} else if (xmlTextReaderNodeType (context->reader) == XML_READER_TYPE_END_ELEMENT)
		idol_cmml_parse_end (context, tag);
	xmlFree (tag);
}

static void
idol_cmml_read_clip_cb (IdolCmmlClip	*clip,
			 gpointer	user_data)
{
	IdolCmmlClip	*new_clip;

	g_return_if_fail (clip != NULL);
	g_return_if_fail (user_data != NULL);

	new_clip = idol_cmml_clip_copy (clip);
	...
	/* clip with -1 start time is bad one, remove it */
		...
		idol_cmml_clip_free (new_clip);
}

/**
 * idol_cmml_convert_msecs_to_str:
 ...
 **/
gchar *
idol_cmml_convert_msecs_to_str (gint64 time_msecs)
{
...
static int
idol_cmml_compare_clips (gconstpointer pointer_a,
			  gconstpointer pointer_b)
{
	IdolCmmlClip	*clip_a, *clip_b;

	g_return_val_if_fail (pointer_a != NULL && pointer_b != NULL, -1);

	clip_a = (IdolCmmlClip *) pointer_a;
	clip_b = (IdolCmmlClip *) pointer_b;
 ...
 *
 * Returns: newly allocated #IdolCmmlClip structure.
 **/
IdolCmmlClip *
idol_cmml_clip_new (const gchar	*title,
		     const gchar	*desc,
		     gint64		start,
		     GdkPixbuf		*pixbuf)
{
...
/**
 * idol_cmml_clip_free:
 * @clip: #IdolCmmlClip to free
 *
 * Frees unused clip structure.
 **/
void
idol_cmml_clip_free (IdolCmmlClip *clip)
{
	if (clip == NULL)
		return;
...
/**
 * idol_cmml_clip_copy:
 * @clip: #IdolCmmlClip structure to copy
 *
 * Copies #IdolCmmlClip structure.
 *
 * Returns: newly allocated #IdolCmmlClip if @clip != %NULL, %NULL otherwise.
 **/
IdolCmmlClip *
idol_cmml_clip_copy (IdolCmmlClip *clip)
{
	g_return_val_if_fail (clip != NULL, NULL);

	return idol_cmml_clip_new (clip->title, clip->desc, clip->time_start, clip->pixbuf);
}

static void
idol_cmml_read_file_result (GObject		*source_object,
			     GAsyncResult	*result,
			     gpointer		user_data)
{
	GError			*error = NULL;
	xmlTextReaderPtr	reader;
	IdolCmmlContext	*context;
	...
	data = (IdolCmmlAsyncData *) user_data;
	data->is_exists = TRUE;
	...
	context = idol_cmml_context_new ();
	context->reader = reader;
	idol_cmml_context_set_callback (context, idol_cmml_read_clip_cb, &(data->list));

	ret = xmlTextReaderRead (reader);
	while (ret == 1) {
		idol_cmml_parse_xml_node (context);
		ret = xmlTextReaderRead (reader);
	}
	...
	idol_cmml_clip_free (context->clip);
	idol_cmml_context_free (context);

	/* sort clips by time growth */
	data->list = g_list_sort (data->list, (GCompareFunc) idol_cmml_compare_clips);
	data->successful = TRUE;
	(data->final) (data, NULL);
}

/**
 * idol_cmml_read_file_async:
 * @data: #IdolCmmlAsyncData structure with info needed
 *
 ...
 **/
gint
idol_cmml_read_file_async (IdolCmmlAsyncData	*data)
{
	GFile	*gio_file;
	...
	g_file_load_contents_async (gio_file, data->cancellable, idol_cmml_read_file_result, data);
	return 0;
}

static void
idol_cmml_write_file_result (GObject		*source_object,
			      GAsyncResult	*result,
			      gpointer		user_data)
{
	GError			*error = NULL;
 ...
 **/
gint
idol_cmml_write_file_async (IdolCmmlAsyncData *data)
{
	GFile			*gio_file;
	gint			res, len;
	GList			*cur_clip;
	...
	/* CMML preamble */
	res = xmlTextWriterWriteRaw (writer, (const xmlChar *) IDOL_CMML_PREAMBLE);
	if (G_UNLIKELY (res < 0)) {
		xmlBufferFree (buf);
		xmlFreeTextWriter (writer);
		return -1;
	...
	while (cur_clip != NULL) {
		...
		clip = (IdolCmmlClip *) cur_clip->data;
		...
		if (G_LIKELY (((IdolCmmlClip *) cur_clip->data)->pixbuf != NULL)) {
			pixdata = g_new0 (GdkPixdata, 1);

			/* encode and serialize pixbuf data */
			gdk_pixdata_from_pixbuf (pixdata, ((IdolCmmlClip *) cur_clip->data)->pixbuf, TRUE);
	...
	g_file_replace_contents_async (gio_file, data->buf, len, NULL, FALSE,
				       ...
				       (GAsyncReadyCallback) idol_cmml_write_file_result, data);
