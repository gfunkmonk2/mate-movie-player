...
/**
 * SECTION:idol-edit-chapter
 * @short_description: dialog to add new chapters in chapters plugin
 * @stability: Unstable
 * @include: idol-edit-chapter.h
 *
 * #IdolEditChapter dialog is used for entering names of new chapters in the chapters plugin
 **/
...
struct IdolEditChapterPrivate {
	GtkEntry	*title_entry;
	GtkWidget	*container;
};


#define IDOL_EDIT_CHAPTER_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE ((obj), IDOL_TYPE_EDIT_CHAPTER, IdolEditChapterPrivate))

G_DEFINE_TYPE (IdolEditChapter, idol_edit_chapter, GTK_TYPE_DIALOG)

/* GtkBuilder callbacks */
void title_entry_changed_cb (GtkEditable *entry, gpointer user_data);

static void
idol_edit_chapter_class_init (IdolEditChapterClass *klass)
{
	g_type_class_add_private (klass, sizeof (IdolEditChapterPrivate));
}
...
static void
idol_edit_chapter_init (IdolEditChapter *self)
{
	GtkBuilder	*builder;

	self->priv = IDOL_EDIT_CHAPTER_GET_PRIVATE (self);
...
GtkWidget*
idol_edit_chapter_new (void)
{
	IdolEditChapter	*edit_chapter;
	GtkWidget		*dialog_area;

	edit_chapter = IDOL_EDIT_CHAPTER (g_object_new (IDOL_TYPE_EDIT_CHAPTER, NULL));
...
void
idol_edit_chapter_set_title (IdolEditChapter	*edit_chapter,
			      const gchar	*title)
{
	g_return_if_fail (IDOL_IS_EDIT_CHAPTER (edit_chapter));

	gtk_entry_set_text (edit_chapter->priv->title_entry, title);
}

gchar *
idol_edit_chapter_get_title (IdolEditChapter *edit_chapter)
{
	g_return_val_if_fail (IDOL_IS_EDIT_CHAPTER (edit_chapter), NULL);
