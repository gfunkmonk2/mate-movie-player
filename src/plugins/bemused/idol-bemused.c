...
#define IDOL_TYPE_BEMUSED_PLUGIN		(idol_bemused_plugin_get_type ())
#define IDOL_BEMUSED_PLUGIN(o)		(G_TYPE_CHECK_INSTANCE_CAST ((o), IDOL_TYPE_BEMUSED_PLUGIN, IdolBemusedPlugin))
#define IDOL_BEMUSED_PLUGIN_CLASS(k)	(G_TYPE_CHECK_CLASS_CAST((k), IDOL_TYPE_BEMUSED_PLUGIN, IdolBemusedPluginClass))
#define IDOL_IS_BEMUSED_PLUGIN(o)		(G_TYPE_CHECK_INSTANCE_TYPE ((o), IDOL_TYPE_BEMUSED_PLUGIN))
#define IDOL_IS_BEMUSED_PLUGIN_CLASS(k)	(G_TYPE_CHECK_CLASS_TYPE ((k), IDOL_TYPE_BEMUSED_PLUGIN))
#define IDOL_BEMUSED_PLUGIN_GET_CLASS(o)	(G_TYPE_INSTANCE_GET_CLASS ((o), IDOL_TYPE_BEMUSED_PLUGIN, IdolBemusedPluginClass))
...
} IdolBemusedPluginClass;

G_MODULE_EXPORT GType register_idol_plugin		(GTypeModule *module);
GType	idol_bemused_plugin_get_type		(void) G_GNUC_CONST;

static void idol_bemused_plugin_init		(IdolBemusedPlugin *plugin);
static void idol_bemused_plugin_finalize		(GObject *object);
...
IDOL_PLUGIN_REGISTER(IdolBemusedPlugin, idol_bemused_plugin)
...
static void
flush_response (IdolBemusedPlugin *tp, GIOChannel *source)
{
	GError *error = NULL;
...
static void
send_response_flush (IdolBemusedPlugin *tp, GIOChannel *source, const char *resp, gssize len, gboolean flush)
{
	GError *error = NULL;
	gsize written = 0;
...
static void
send_response (IdolBemusedPlugin *tp, GIOChannel *source, const char *resp, gssize len)
{
	send_response_flush (tp, source, resp, len, TRUE);
}

static void
read_response (IdolBemusedPlugin *tp, GIOChannel *source, char *buf, gssize len)
{
	GError *error = NULL;
	gsize read;
...
static char *
read_filename (IdolBemusedPlugin *tp, GIOChannel *source)
{
...
static void
write_playlist (IdolBemusedPlugin *tp, GIOChannel *source)
{
	char buf[11];
	int playlist_pos, playlist_len, i;

	playlist_pos = idol_get_playlist_pos (tp->idol);
	playlist_len = idol_get_playlist_length (tp->idol);
	...
	for (i = 0; i < playlist_len; i++) {
		...
		title = idol_get_title_at_playlist_pos (tp->idol, i);
		if (title == NULL) {
...
static void
write_current_volume (IdolBemusedPlugin *tp, GIOChannel *source)
{
	double volume;
	char buf[8];
...
static void
set_volume (IdolBemusedPlugin *tp, GIOChannel *source)
{
	char buf;
	double volume;
...
static void
write_version (IdolBemusedPlugin *tp, GIOChannel *source)
{
	char buf[10];
...
static void
set_setting (IdolBemusedPlugin *tp, GIOChannel *source, IdolRemoteSetting setting)
{
	char buf;

	read_response (tp, source, &buf, 1);
	idol_action_remote_set_setting (tp->idol, setting, buf != 0);
}

static void
seek_to_pos (IdolBemusedPlugin *tp, GIOChannel *source)
{
	char buf[4];
	int time;
	...
	idol_action_seek_time (tp->idol, (gint64) time * 1000, FALSE);
}

static void
write_playlist_length (IdolBemusedPlugin *tp, GIOChannel *source)
{
	char buf[2];
	int len;

	len = idol_get_playlist_length (tp->idol);
...
static void
add_or_enqueue (IdolBemusedPlugin *tp, GIOChannel *source, IdolRemoteCommand cmd)
{
	char *filename;
	...
	g_message ("%s file '%s'", cmd == IDOL_REMOTE_COMMAND_ENQUEUE ? "Enqueuing" : "Replacing with", filename);
...
static void
download_file (IdolBemusedPlugin *tp, GIOChannel *source)
{
	char *filename;
	GMappedFile *file;
...
static void
write_detailed_song_info (IdolBemusedPlugin *tp, GIOChannel *source)
{
...
static void
write_detailed_file_info (IdolBemusedPlugin *tp, GIOChannel *source)
{
	char *filename;
...
static void
write_song_info (IdolBemusedPlugin *tp, GIOChannel *source, gboolean send_null)
{
	char *title;
	char status;
	...
	if (idol_is_playing (tp->idol) != FALSE)
		status = 1;
	else if (idol_is_paused (tp->idol) != FALSE)
		status = 2;
	else
		status = 0;
	...
	status = idol_action_remote_get_setting (tp->idol, IDOL_REMOTE_SETTING_SHUFFLE);
	send_response_flush (tp, source, &status, 1, FALSE);
	status = idol_action_remote_get_setting (tp->idol, IDOL_REMOTE_SETTING_REPEAT);
	send_response_flush (tp, source, &status, 1, FALSE);

	title = idol_get_short_title (tp->idol);
...
static void
set_playlist_at_pos (IdolBemusedPlugin *tp, GIOChannel *source)
{
	char buf[2];
	int index;
	...
	index = (buf[0] << 8) + buf[1];
	idol_action_set_playlist_index (tp->idol, index);
}

#if 0
static void
write_directory_listing_at_dir (IdolBemusedPlugin *tp, GIOChannel *source, const char *directory)
{
}
#endif
static void
write_directory_listing (IdolBemusedPlugin *tp, GIOChannel *source, gboolean at_root)
{
	char *filename;
	char buf[2];
...
static void
handle_command (IdolBemusedPlugin *tp, GIOChannel *source, const char *cmd)
{
	g_message ("cmd: %s", cmd);
	...
	CMD_IS("CHCK") {
		...
		idol_action_remote (tp->idol, IDOL_REMOTE_COMMAND_SEEK_FORWARD, NULL);
	} else CMD_IS("FINF") {
		...
		add_or_enqueue (tp, source, IDOL_REMOTE_COMMAND_ENQUEUE);
	} else CMD_IS("LIST") {
		write_directory_listing (tp, source, TRUE);
	} else CMD_IS("NEXT") {
		idol_action_remote (tp->idol, IDOL_REMOTE_COMMAND_NEXT, NULL);
	} else CMD_IS("PAUS") {
		idol_action_remote (tp->idol, IDOL_REMOTE_COMMAND_PAUSE, NULL);
	} else CMD_IS("PLAY") {
		add_or_enqueue (tp, source, IDOL_REMOTE_COMMAND_REPLACE);
	} else CMD_IS("PLEN") {
		write_playlist_length (tp, source);
	} else CMD_IS("PLST") {
		write_playlist (tp, source);
	} else CMD_IS("PREV") {
		idol_action_remote (tp->idol, IDOL_REMOTE_COMMAND_PREVIOUS, NULL);
	} else CMD_IS("REPT") {
		set_setting (tp, source, IDOL_REMOTE_SETTING_REPEAT);
	} else CMD_IS("RMAL") {
		idol_action_remote (tp->idol, IDOL_REMOTE_COMMAND_REPLACE, NULL);
	} else CMD_IS("RWND") {
		idol_action_remote (tp->idol, IDOL_REMOTE_COMMAND_SEEK_BACKWARD, NULL);
	} else CMD_IS("SHFL") {
		set_setting (tp, source, IDOL_REMOTE_SETTING_SHUFFLE);
	} else CMD_IS("SEEK") {
		seek_to_pos (tp, source);
	} else CMD_IS("SHUT") {
		idol_action_remote (tp->idol, IDOL_REMOTE_COMMAND_QUIT, NULL);
	} else CMD_IS("STEN") {
		...
		idol_action_remote (tp->idol, IDOL_REMOTE_COMMAND_PLAY, NULL);
	} else CMD_IS("VOLM") {
...
{
	IdolBemusedPlugin *tp = (IdolBemusedPlugin *) data;
	GIOStatus status;
...
server_watch_func (GIOChannel *source, GIOCondition condition, gpointer data)
{
	...
	if (condition & G_IO_IN) {
		...
		if (client_fd < 0) {
			...
			tp->bvw = BACON_VIDEO_WIDGET (idol_get_video_widget (tp->idol));
...
#define BEMUSED_SVC_NAME N_("Idol Bemused Server")
//FIXME version
#define BEMUSED_SVC_DESC N_("Idol Bemused Server version 1.0")
#define BEMUSED_SVC_PROV "http://www.mate.org/projects/idol/"
...
static void
idol_bemused_plugin_class_init (IdolBemusedPluginClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);
	IdolPluginClass *plugin_class = IDOL_PLUGIN_CLASS (klass);

	object_class->finalize = idol_bemused_plugin_finalize;
...
static void
idol_bemused_plugin_init (IdolBemusedPlugin *plugin)
{
}

static void
idol_bemused_plugin_finalize (GObject *object)
{
	G_OBJECT_CLASS (idol_bemused_plugin_parent_class)->finalize (object);
}
...
impl_activate (IdolPlugin *plugin,
	       ...
	       GError **error)
{
	IdolBemusedPlugin *tp = IDOL_BEMUSED_PLUGIN (plugin);
	int fd, channel;
