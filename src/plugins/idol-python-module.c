...
#define IDOL_PYTHON_MODULE_GET_PRIVATE(object) (G_TYPE_INSTANCE_GET_PRIVATE ((object), \
						 IDOL_TYPE_PYTHON_MODULE, \
						 IdolPythonModulePrivate))
 ...
 * idol_python_module_init_python were successful */
...
/* Exported by Idol Python module */
void pyidol_register_classes (PyObject *d);
void pyidol_add_constants (PyObject *module, const gchar *strip_prefix);
extern PyMethodDef pyidol_functions[];

/* We retreive this to check for correct class hierarchy */
static PyTypeObject *PyIdolPlugin_Type;

G_DEFINE_TYPE (IdolPythonModule, idol_python_module, G_TYPE_TYPE_MODULE);

static void
idol_python_module_init_python (void)
{
	PyObject *pygtk, *mdict, *require;
	PyObject *idol, *gtk, *pygtk_version, *pygtk_required_version;
	 ...
	 * the crash reported on pluma bug #326191 */
	...
	idol = PyImport_ImportModule ("idol");

	if (idol == NULL) {
		g_warning ("Could not import Python module 'idol', check your installation");
		PyErr_Print ();
		return;
	}

	/* add pyidol_functions */
	for (res = 0; pyidol_functions[res].ml_name != NULL; res++) {
		PyObject *func;

		func = PyCFunction_New (&pyidol_functions[res], idol);
		if (func == NULL) {
			g_warning ("unable object for function '%s' create", pyidol_functions[res].ml_name);
			PyErr_Print ();
			return;
		}
		if (PyModule_AddObject (idol, pyidol_functions[res].ml_name, func) < 0) {
			g_warning ("unable to insert function '%s' in 'idol' module", pyidol_functions[res].ml_name);
			PyErr_Print ();
			return;
		}
	}
	mdict = PyModule_GetDict (idol);

	pyidol_register_classes (mdict);
	pyidol_add_constants (idol, "IDOL_");

	/* Retreive the Python type for idol.Plugin */
	PyIdolPlugin_Type = (PyTypeObject *) PyDict_GetItemString (mdict, "Plugin");
	if (PyIdolPlugin_Type == NULL) {
		PyErr_Print ();
		return;
	}
	...
	/* ideally idol should clean up the python stuff again at some point,
	 * for which we'd need to save the result of SaveThread so we can then
	 * restore the state in a class_finalize or so, but since idol doesn't
...
static gboolean
idol_python_module_load (GTypeModule *gmodule)
{
	IdolPythonModulePrivate *priv = IDOL_PYTHON_MODULE_GET_PRIVATE (gmodule);
	...
	while (PyDict_Next (locals, &pos, &key, &value))
	{
		...
		if (PyObject_IsSubclass (value, (PyObject*) PyIdolPlugin_Type))
		{
			priv->type = idol_python_object_get_type (gmodule, value);
			res = TRUE;
			goto done;
		}
	}

	g_warning ("Failed to find any idol.Plugin-derived classes in Python plugin");
...
static void
idol_python_module_unload (GTypeModule *module)
{
	IdolPythonModulePrivate *priv = IDOL_PYTHON_MODULE_GET_PRIVATE (module);
...
GObject *
idol_python_module_new_object (IdolPythonModule *module)
{
	IdolPythonModulePrivate *priv = IDOL_PYTHON_MODULE_GET_PRIVATE (module);
	IdolPythonObject *object;

	if (priv->type == 0)
		return NULL;

	g_debug ("Creating object of type %s", g_type_name (priv->type));
	object = (IdolPythonObject*) (g_object_new (priv->type,
						  "name", priv->module,
						  NULL));
	 ...
	 *
	 * This still has issues, notably that it isn't safe to call any idol.Plugin methods
...
static void
idol_python_module_init (IdolPythonModule *module)
{
	g_debug ("Init of Python module");
}

static void
idol_python_module_finalize (GObject *object)
{
	IdolPythonModulePrivate *priv = IDOL_PYTHON_MODULE_GET_PRIVATE (object);
	...
	G_OBJECT_CLASS (idol_python_module_parent_class)->finalize (object);
}

static void
idol_python_module_get_property (GObject    *object,
				  guint       prop_id,
				  GValue     *value,
				  GParamSpec *pspec)
{
...
static void
idol_python_module_set_property (GObject      *object,
				  guint         prop_id,
				  const GValue *value,
				  GParamSpec   *pspec)
{
	IdolPythonModule *mod = IDOL_PYTHON_MODULE (object);

	switch (prop_id)
	{
		case PROP_MODULE:
			IDOL_PYTHON_MODULE_GET_PRIVATE (mod)->module = g_value_dup_string (value);
			break;
		case PROP_PATH:
			IDOL_PYTHON_MODULE_GET_PRIVATE (mod)->path = g_value_dup_string (value);
			break;
		default:
			g_return_if_reached ();
	}
...
static void
idol_python_module_class_init (IdolPythonModuleClass *class)
{
	GObjectClass *object_class = G_OBJECT_CLASS (class);
	GTypeModuleClass *module_class = G_TYPE_MODULE_CLASS (class);

	object_class->finalize = idol_python_module_finalize;
	object_class->get_property = idol_python_module_get_property;
	object_class->set_property = idol_python_module_set_property;
	...
	g_type_class_add_private (object_class, sizeof (IdolPythonModulePrivate));

	module_class->load = idol_python_module_load;
	module_class->unload = idol_python_module_unload;
	...
	idol_python_module_init_python ();
}

IdolPythonModule *
idol_python_module_new (const gchar *path,
			 const gchar *module)
{
	IdolPythonModule *result;
	gchar *dir;
	...
	result = g_object_new (IDOL_TYPE_PYTHON_MODULE,
			       "module", module,
			       "path", dir,
			       NULL);
	g_free (dir);
...
void
idol_python_garbage_collect (void)
{
#if 0
