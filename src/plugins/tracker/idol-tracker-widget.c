...
#define IDOL_TRACKER_MAX_RESULTS_SIZE	20

G_DEFINE_TYPE (IdolTrackerWidget, idol_tracker_widget, GTK_TYPE_VBOX)

struct IdolTrackerWidgetPrivate {
	...
	IdolVideoList *result_list;
};

typedef struct {
	IdolTrackerWidget *widget;
...
static void idol_tracker_widget_dispose	(GObject *object);
static void idol_tracker_widget_set_property	(GObject *object,
						 guint property_id,
						 const GValue *value,
						 GParamSpec *pspec);
static void page_selector_value_changed_cb (GtkSpinButton *self, IdolTrackerWidget *widget);

static void
idol_tracker_widget_class_init (IdolTrackerWidgetClass *klass)
{
	GObjectClass *object_class;

	g_type_class_add_private (klass, sizeof (IdolTrackerWidgetPrivate));

	object_class = G_OBJECT_CLASS (klass);
	object_class->dispose = idol_tracker_widget_dispose;
	object_class->set_property = idol_tracker_widget_set_property;

	g_object_class_install_property (object_class, PROP_IDOL,
					 g_param_spec_object ("idol", NULL, NULL,
							      IDOL_TYPE_OBJECT, G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));
}

static void
idol_tracker_widget_dispose (GObject *object)
{
	IdolTrackerWidget *self = IDOL_TRACKER_WIDGET (object);
	...
	G_OBJECT_CLASS (idol_tracker_widget_parent_class)->dispose (object);
}

static void
idol_tracker_widget_set_property (GObject *object,
				   guint property_id,
				   const GValue *value,
				   GParamSpec *pspec)
{
	...
	widget = IDOL_TRACKER_WIDGET (object);

	switch (property_id)
	{
	case PROP_IDOL:
		widget->idol = g_value_dup_object (value);
		g_object_set (G_OBJECT (widget->priv->result_list), "idol", widget->idol, NULL);
		break;
	default:
...
search_results_new (IdolTrackerWidget *widget,
...
static void
do_search (IdolTrackerWidget *widget)
{
	SearchResultsData *srd;
	GPtrArray *results;
	GError *error = NULL;
	...
	offset = widget->priv->current_result_page * IDOL_TRACKER_MAX_RESULTS_SIZE;
...
static void
new_search (GtkButton *button, IdolTrackerWidget *widget)
{
...
static void
init_result_list (IdolTrackerWidget *widget)
{
	...
	widget->priv->result_list = g_object_new (IDOL_TYPE_VIDEO_LIST,
...
static void
initialize_list_store (IdolTrackerWidget *widget) 
{
...
search_entry_changed_cb (GtkWidget *entry,
			 gpointer   user_data)
{
	IdolTrackerWidget *widget = user_data;

	gtk_label_set_text (GTK_LABEL (widget->priv->status_label), "");
}

static void
idol_tracker_widget_init (IdolTrackerWidget *widget)
{
	...
	widget->priv = G_TYPE_INSTANCE_GET_PRIVATE (widget, IDOL_TYPE_TRACKER_WIDGET, IdolTrackerWidgetPrivate);
...
GtkWidget *
idol_tracker_widget_new (IdolObject *idol)
{
	GtkWidget *widget;

	widget = g_object_new (IDOL_TYPE_TRACKER_WIDGET,
			       "idol", idol, NULL);

	/* Reset the info about the search */
	IDOL_TRACKER_WIDGET (widget)->priv->total_result_count = 0;
	IDOL_TRACKER_WIDGET (widget)->priv->current_result_page = 0;

	initialize_list_store (IDOL_TRACKER_WIDGET (widget));
