...
static void idol_skipto_dispose	(GObject *object);

/* Callback functions for GtkBuilder */
G_MODULE_EXPORT void time_entry_activate_cb (GtkEntry *entry, IdolSkipto *skipto);
G_MODULE_EXPORT void tstw_adjustment_value_changed_cb (GtkAdjustment *adjustment, IdolSkipto *skipto);

struct IdolSkiptoPrivate {
	GtkBuilder *xml;
...
IDOL_PLUGIN_DEFINE_TYPE (IdolSkipto, idol_skipto, GTK_TYPE_DIALOG)
#define IDOL_SKIPTO_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE ((obj), IDOL_TYPE_SKIPTO, IdolSkiptoPrivate))

static void
idol_skipto_class_init (IdolSkiptoClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	g_type_class_add_private (klass, sizeof (IdolSkiptoPrivate));

	object_class->dispose = idol_skipto_dispose;
}

static void
idol_skipto_response_cb (GtkDialog *dialog, gint response_id, gpointer data)
{
	IdolSkipto *skipto;

	skipto = IDOL_SKIPTO (dialog);
	gtk_spin_button_update (GTK_SPIN_BUTTON (skipto->priv->time_entry));
}

static void
idol_skipto_init (IdolSkipto *skipto)
{
	skipto->priv = G_TYPE_INSTANCE_GET_PRIVATE (skipto, IDOL_TYPE_SKIPTO, IdolSkiptoPrivate);

	gtk_dialog_set_default_response (GTK_DIALOG (skipto), GTK_RESPONSE_OK);
	g_signal_connect (skipto, "response",
				G_CALLBACK (idol_skipto_response_cb), NULL);
}

static void
idol_skipto_dispose (GObject *object)
{
	IdolSkiptoPrivate *priv = IDOL_SKIPTO_GET_PRIVATE (object);
	...
	G_OBJECT_CLASS (idol_skipto_parent_class)->dispose (object);
}

void
idol_skipto_update_range (IdolSkipto *skipto, gint64 _time)
{
	g_return_if_fail (IDOL_IS_SKIPTO (skipto));
...
gint64
idol_skipto_get_range (IdolSkipto *skipto)
{
	gint64 _time;

	g_return_val_if_fail (IDOL_IS_SKIPTO (skipto), 0);
...
void
idol_skipto_set_seekable (IdolSkipto *skipto, gboolean seekable)
{
	g_return_if_fail (IDOL_IS_SKIPTO (skipto));
...
void
idol_skipto_set_current (IdolSkipto *skipto, gint64 _time)
{
	g_return_if_fail (IDOL_IS_SKIPTO (skipto));
...
void
time_entry_activate_cb (GtkEntry *entry, IdolSkipto *skipto)
{
	gtk_dialog_response (GTK_DIALOG (skipto), GTK_RESPONSE_OK);
}

void
tstw_adjustment_value_changed_cb (GtkAdjustment *adjustment, IdolSkipto *skipto)
{
...
GtkWidget *
idol_skipto_new (IdolSkiptoPlugin *plugin)
{
	IdolSkipto *skipto;
	GtkWidget *container;

	skipto = IDOL_SKIPTO (g_object_new (IDOL_TYPE_SKIPTO, NULL));

	skipto->priv->idol = plugin->idol;
	skipto->priv->xml = idol_plugin_load_interface (IDOL_PLUGIN (plugin),
							 "skipto.ui", TRUE,
							 NULL, skipto);
	...
	gtk_window_set_transient_for (GTK_WINDOW (skipto),
				      idol_get_main_window (plugin->idol));
