...
G_DEFINE_TYPE (IdolTimeEntry, idol_time_entry, GTK_TYPE_SPIN_BUTTON)

static void
idol_time_entry_class_init (IdolTimeEntryClass *klass)
{
	/* Nothing to see here; please move along */
}

static void
idol_time_entry_init (IdolTimeEntry *self)
{
	/* Connect to signals */
...
GtkWidget *
idol_time_entry_new (GtkAdjustment *adjustment, gdouble climb_rate)
{
	return g_object_new (IDOL_TYPE_TIME_ENTRY,
...
output_cb (GtkSpinButton *self, gpointer user_data)
{
	...
	text = idol_time_to_string ((gint64) gtk_spin_button_get_value (self) * 1000);
...
input_cb (GtkSpinButton *self, gdouble *new_value, gpointer user_data)
{
	...
	val = idol_string_to_time (gtk_entry_get_text (GTK_ENTRY (self)));
	if (val == -1)
