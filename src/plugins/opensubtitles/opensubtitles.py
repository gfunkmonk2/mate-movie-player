...
USER_AGENT = 'Idol'
OK200 = '200 OK'
IDOL_REMOTE_COMMAND_REPLACE = 14
...
class OpenSubtitles(idol.Plugin):
    def __init__(self):
    ...
    def activate(self, idol_object):
        ...
        the callbacks, ...) and added to idol.

        @param idol_object:
        @type  idol_object: {idol.IdolObject}
        """
        self.idol = idol_object
	self.filename = None

        self.manager = self.idol.get_ui_manager()
        self.os_append_menu()

        self.idol.connect('file-opened', self.on_idol__file_opened)
        self.idol.connect('file-closed', self.on_idol__file_closed)
    ...
    def os_build_dialog(self, action, idol_object):
        builder = self.load_interface("opensubtitles.ui", 
                                       True, 
                                       self.idol.get_main_window(), 
                                       self)
	...
	self.dialog.set_transient_for (self.idol.get_main_window())
    ...
    def os_show_dialog(self, action, idol_object):
        if not self.dialog:
            self.os_build_dialog(action, idol_object)

        filename = self.idol.get_current_mrl()
    ...
    def os_append_menu(self):
        ...
        self.action.connect('activate', self.os_show_dialog, self.idol)

        self.action.set_sensitive(self.idol.is_playing() and
				  self.os_check_allowed_scheme() and
                                  not self.os_check_is_audio())

    def os_check_allowed_scheme(self):
        scheme = gio.File(self.idol.get_current_mrl()).get_uri_scheme()
    ...
    def os_save_selected_subtitle(self, filename=None):
        ...
        if rows:
            ...
            if not filename:
                directory = gio.File(xdg.BaseDirectory.xdg_cache_home + sep + 'idol' + sep + 'subtitles' + sep) 
                if not directory.query_exists():
                    if not path.exists (xdg.BaseDirectory.xdg_cache_home + sep + 'idol' + sep):
                        mkdir (xdg.BaseDirectory.xdg_cache_home + sep + 'idol' + sep)
                    if not path.exists (xdg.BaseDirectory.xdg_cache_home + sep + 'idol' + sep + 'subtitles' + sep):
                        mkdir (xdg.BaseDirectory.xdg_cache_home + sep + 'idol' + sep + 'subtitles' + sep)
    ...
    def os_save_subtitles(self, filename):
        ...
        if suburi:
            self.idol.set_current_subtitle(suburi)
    ...
    def on_idol__file_opened(self, idol, filename):
        """
        """
	...
	if self.os_check_allowed_scheme() and not self.os_check_is_audio():
	    ...
	    if self.dialog:
	    	self.find_button.set_sensitive(True)
		self.filename = self.idol.get_current_mrl()
		self.liststore.clear()
	        self.treeview.set_headers_visible(False)
	    	self.apply_button.set_sensitive(False)
		self.results = [] 
    ...
    def on_idol__file_closed(self, idol):
        self.action.set_sensitive(False)
        if self.dialog:
	    self.apply_button.set_sensitive(False)
	    self.find_button.set_sensitive(False)
