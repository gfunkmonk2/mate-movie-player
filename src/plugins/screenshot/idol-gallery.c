...
#include "idol-gallery-progress.h"
#include "idol-screenshot-plugin.h"

static void dialog_response_callback (GtkDialog *dialog, gint response_id, IdolGallery *self);

/* GtkBuilder callbacks */
void default_screenshot_count_toggled_callback (GtkToggleButton *toggle_button, IdolGallery *self);

struct _IdolGalleryPrivate {
	Idol *idol;
...
G_DEFINE_TYPE (IdolGallery, idol_gallery, GTK_TYPE_FILE_CHOOSER_DIALOG)
#define IDOL_GALLERY_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE ((obj), IDOL_TYPE_GALLERY, IdolGalleryPrivate))

static void
idol_gallery_class_init (IdolGalleryClass *klass)
{
	g_type_class_add_private (klass, sizeof (IdolGalleryPrivate));
}

static void
idol_gallery_init (IdolGallery *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, IDOL_TYPE_GALLERY, IdolGalleryPrivate);
}

IdolGallery *
idol_gallery_new (Idol *idol, IdolPlugin *plugin)
{
	IdolGallery *gallery;
	GtkWidget *container;
	GtkBuilder *builder;
	...
	gallery = g_object_new (IDOL_TYPE_GALLERY, NULL);

	builder = idol_plugin_load_interface (plugin, "gallery.ui", TRUE, NULL, gallery);
	if (builder == NULL) {
	 ...
	 * "Galerie-%s-%d.jpg". */
	uri = idol_screenshot_plugin_setup_file_chooser (N_("Gallery-%s-%d.jpg"), movie_title);
	g_free (movie_title);
...
void
default_screenshot_count_toggled_callback (GtkToggleButton *toggle_button, IdolGallery *self)
{
...
static void
dialog_response_callback (GtkDialog *dialog, gint response_id, IdolGallery *self)
{
	...
	video_mrl = idol_get_current_mrl (self->priv->idol);
	idol_screenshot_plugin_update_file_chooser (filename);

	/* Build the command and arguments to pass it */
	argv[0] = (gchar*) "idol-video-thumbnailer"; /* a little hacky, but only the allocated stuff is freed below */
	...
	if (ret == FALSE) {
		g_warning ("Error spawning idol-video-thumbnailer: %s", error->message);
		g_error_free (error);
		return;
	}

	/* Create the progress dialogue */
	progress_dialog = GTK_WIDGET (idol_gallery_progress_new (child_pid, filename));
	g_free (filename);
	idol_gallery_progress_run (IDOL_GALLERY_PROGRESS (progress_dialog), stdout_fd);
