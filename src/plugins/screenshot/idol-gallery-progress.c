...
static void idol_gallery_progress_finalize (GObject *object);
static void dialog_response_callback (GtkDialog *dialog, gint response_id, IdolGalleryProgress *self);

struct _IdolGalleryProgressPrivate {
	GPid child_pid;
	GString *line;
...
G_DEFINE_TYPE (IdolGalleryProgress, idol_gallery_progress, GTK_TYPE_DIALOG)
#define IDOL_GALLERY_PROGRESS_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE ((obj), IDOL_TYPE_GALLERY_PROGRESS, IdolGalleryProgressPrivate))

static void
idol_gallery_progress_class_init (IdolGalleryProgressClass *klass)
{
	GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

	g_type_class_add_private (klass, sizeof (IdolGalleryProgressPrivate));

	gobject_class->finalize = idol_gallery_progress_finalize;
}

static void
idol_gallery_progress_init (IdolGalleryProgress *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, IDOL_TYPE_GALLERY_PROGRESS, IdolGalleryProgressPrivate);
}

static void
idol_gallery_progress_finalize (GObject *object)
{
	IdolGalleryProgressPrivate *priv = IDOL_GALLERY_PROGRESS_GET_PRIVATE (object);
	...
	G_OBJECT_CLASS (idol_gallery_progress_parent_class)->finalize (object);
}

IdolGalleryProgress *
idol_gallery_progress_new (GPid child_pid, const gchar *output_filename)
{
	IdolGalleryProgress *self;
	GtkWidget *container;
	gchar *label_text;

	/* Create the gallery */
	self = g_object_new (IDOL_TYPE_GALLERY_PROGRESS, NULL);
...
static void
dialog_response_callback (GtkDialog *dialog, gint response_id, IdolGalleryProgress *self)
{
...
static gboolean
process_line (IdolGalleryProgress *self, const gchar *line)
{
...
static gboolean
stdout_watch_cb (GIOChannel *source, GIOCondition condition, IdolGalleryProgress *self)
{
	/* Code pilfered from caja-burn-process.c (caja-cd-burner) under GPLv2+
	 * Copyright (C) 2006 William Jon McCann <mccann@jhu.edu> */
	IdolGalleryProgressPrivate *priv = self->priv;
...
void
idol_gallery_progress_run (IdolGalleryProgress *self, gint stdout_fd)
{
	GIOChannel *channel;

	fcntl (stdout_fd, F_SETFL, O_NONBLOCK);

	/* Watch the output from idol-video-thumbnailer */
