...
class IplayerPlugin (idol.Plugin):
	def __init__ (self):
	...
	def deactivate (self, idol_object):
		idol_object.remove_sidebar_page ("iplayer")
	...
	def _populate_channel_list_cb (self, tree_store, parent_path, values):
		...
		if values == None:
			self.idol.action_error (_('Error listing channel categories'), _('There was an unknown error getting the list of television channels available on BBC iPlayer.'))
			return False
	...
	def _row_activated_cb (self, tree_view, path, view_column):
		...
		self.idol.add_to_playlist_and_play (mrl, tree_store.get_value (tree_iter, 0), True)
	...
	def _populate_programme_list_cb (self, tree_store, category_path, values, remove_placeholder):
		...
		if values == None:
			self.idol.action_error (_('Error getting programme feed'), _('There was an error getting the list of programmes for this channel and category combination.'))
			return False
