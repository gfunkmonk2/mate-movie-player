...
class PythonConsolePlugin(idol.Plugin):
	def __init__(self):
	...
	def activate(self, idol_object):
		...
		data = dict()
		manager = idol_object.get_ui_manager()
		...
		action = gtk.Action('PythonConsole', _('_Python Console'),
		                    _("Show Idol's Python console"),
		                    'mate-mime-text-x-python')
		action.connect('activate', self.show_console, idol_object)
		data['action_group'].add_action(action)
		...
		action = gtk.Action('PythonDebugger', _('Python Debugger'),
				    ...
				    None)
		if have_rpdb2:
			action.connect('activate', self.enable_debugging, idol_object)
		else:
		...
		idol_object.set_data('PythonConsolePluginInfo', data)
	...
	def enable_debugging(self, action, idol_object):
		msg = _("After you press OK, Idol will wait until you connect to it with winpdb or rpdb2. If you have not set a debugger password in MateConf, it will use the default password ('idol').")
		...
		if dialog.run() == gtk.RESPONSE_OK:
			...
			password = mateconfclient.get_string('/apps/idol/plugins/pythonconsole/rpdb2_password') or "idol"
	...
	def deactivate(self, idol_object):
		data = idol_object.get_data('PythonConsolePluginInfo')
		...
		idol_object.set_data('PythonConsolePluginInfo', None)
