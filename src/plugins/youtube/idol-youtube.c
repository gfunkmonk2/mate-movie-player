...
#define CLIENT_ID	"ytapi-GNOME-Idol-444fubtt-1"
#define MAX_RESULTS	10
#define THUMBNAIL_WIDTH	180
#define PULSE_INTERVAL	200

#define IDOL_TYPE_YOUTUBE_PLUGIN		(idol_youtube_plugin_get_type ())
#define IDOL_YOUTUBE_PLUGIN(o)			(G_TYPE_CHECK_INSTANCE_CAST ((o), IDOL_TYPE_YOUTUBE_PLUGIN, IdolYouTubePlugin))
#define IDOL_YOUTUBE_PLUGIN_CLASS(k)		(G_TYPE_CHECK_CLASS_CAST((k), IDOL_TYPE_YOUTUBE_PLUGIN, IdolYouTubePluginClass))
#define IDOL_IS_YOUTUBE_PLUGIN(o)		(G_TYPE_CHECK_INSTANCE_TYPE ((o), IDOL_TYPE_YOUTUBE_PLUGIN))
#define IDOL_IS_YOUTUBE_PLUGIN_CLASS(k)	(G_TYPE_CHECK_CLASS_TYPE ((k), IDOL_TYPE_YOUTUBE_PLUGIN))
#define IDOL_YOUTUBE_PLUGIN_GET_CLASS(o)	(G_TYPE_INSTANCE_GET_CLASS ((o), IDOL_TYPE_YOUTUBE_PLUGIN, IdolYouTubePluginClass))
...
} IdolYouTubePluginClass;

G_MODULE_EXPORT GType register_idol_plugin	(GTypeModule *module);
GType idol_youtube_plugin_get_type		(void) G_GNUC_CONST;
...
void notebook_switch_page_cb (GtkNotebook *notebook, gpointer *page, guint page_num, IdolYouTubePlugin *self);
void search_button_clicked_cb (GtkButton *button, IdolYouTubePlugin *self);
void cancel_button_clicked_cb (GtkButton *button, IdolYouTubePlugin *self);
void search_entry_activate_cb (GtkEntry *entry, IdolYouTubePlugin *self);
gboolean button_press_event_cb (GtkWidget *widget, GdkEventButton *event, IdolYouTubePlugin *self);
gboolean button_release_event_cb (GtkWidget *widget, GdkEventButton *event, IdolYouTubePlugin *self);
void open_in_web_browser_activate_cb (GtkAction *action, IdolYouTubePlugin *self);
void value_changed_cb (GtkAdjustment *adjustment, IdolYouTubePlugin *self);
gboolean starting_video_cb (IdolVideoList *video_list, GtkTreePath *path, IdolYouTubePlugin *self);

IDOL_PLUGIN_REGISTER (IdolYouTubePlugin, idol_youtube_plugin)

static void
idol_youtube_plugin_class_init (IdolYouTubePluginClass *klass)
{
...
static void
idol_youtube_plugin_init (IdolYouTubePlugin *plugin)
{
 ...
 **/
static void
idol_gdk_pixbuf_new_from_stream_at_scale_async (GInputStream        *stream,
					   gint                 width,
					   gint                 height,
					   gboolean             preserve_aspect_ratio,
					   GCancellable        *cancellable,
					   ...
					   gpointer             user_data)
{
	...
	result = g_simple_async_result_new (G_OBJECT (stream), callback, user_data, idol_gdk_pixbuf_new_from_stream_at_scale_async);
 ...
 **/
static void
idol_gdk_pixbuf_new_from_stream_async (GInputStream        *stream,
				  GCancellable        *cancellable,
				  GAsyncReadyCallback  callback,
				  gpointer             user_data)
{
	...
	result = g_simple_async_result_new (G_OBJECT (stream), callback, user_data, idol_gdk_pixbuf_new_from_stream_async);
	g_simple_async_result_run_in_thread (result, (GSimpleAsyncThreadFunc) new_from_stream_thread, G_PRIORITY_DEFAULT, cancellable);
	g_object_unref (result);
}
 ...
 **/
static GdkPixbuf *
idol_gdk_pixbuf_new_from_stream_finish (GAsyncResult  *async_result,
				   GError       **error)
{
	GdkPixbuf *pixbuf;
	...
	g_warn_if_fail (g_simple_async_result_get_source_tag (result) == idol_gdk_pixbuf_new_from_stream_async ||
			g_simple_async_result_get_source_tag (result) == idol_gdk_pixbuf_new_from_stream_at_scale_async);
...
static void
set_up_tree_view (IdolYouTubePlugin *self, GtkBuilder *builder, guint key)
{
	...
	renderer = GTK_CELL_RENDERER (idol_cell_renderer_video_new (TRUE));
	...
	/* Give the video lists a handle to Idol and connect their scrollbar signals */
	...
	g_object_set (tree_view, "idol", self->idol, NULL);
	...
	ui_manager = idol_video_list_get_ui_manager (IDOL_VIDEO_LIST (tree_view));
	...
	gtk_ui_manager_add_ui (ui_manager, gtk_ui_manager_new_merge_id (ui_manager),
			       "/ui/idol-video-list-popup/",
			       "open-in-web-browser",
			       "open-in-web-browser",
			       GTK_UI_MANAGER_MENUITEM,
			       FALSE);

	menu_item = gtk_ui_manager_get_action (ui_manager, "/ui/idol-video-list-popup/open-in-web-browser");
...
{
	IdolYouTubePlugin *self = IDOL_YOUTUBE_PLUGIN (plugin);
	GtkWindow *main_window;
	GtkBuilder *builder;
	guint i;

	self->idol = g_object_ref (idol);
	self->bvw = BACON_VIDEO_WIDGET (idol_get_video_widget (idol));

	/* Set up the interface */
	main_window = idol_get_main_window (idol);
	builder = idol_plugin_load_interface (plugin, "youtube.ui", TRUE, main_window, self);
	...
	idol_add_sidebar_page (idol, "youtube", _("YouTube"), self->vbox);
...
impl_deactivate	(IdolPlugin *plugin, IdolObject *idol)
{
	...
	idol_remove_sidebar_page (self->idol, "youtube");
...
{
	IdolYouTubePlugin *self = data->plugin;
...
static void
set_progress_bar_text (IdolYouTubePlugin *self, const gchar *text, guint tree_view)
{
...
static void
increment_progress_bar_fraction (IdolYouTubePlugin *self, guint tree_view)
{
...
resolve_t_param_cb (SoupSession *session, SoupMessage *message, TParamData *data)
{
	...
	if (message->status_code != SOUP_STATUS_OK) {
		...
		window = idol_get_main_window (data->plugin->idol);
		idol_interface_error (_("Error Looking Up Video URI"), message->response_body->data, window);
...
static void
resolve_t_param (IdolYouTubePlugin *self, GDataEntry *entry, GtkTreeIter *iter, guint tree_view, GCancellable *cancellable)
{
	GDataLink *page_link;
	TParamData *data;
...
thumbnail_loaded_cb (GObject *source_object, GAsyncResult *result, ThumbnailData *data)
{
	...
	thumbnail = idol_gdk_pixbuf_new_from_stream_finish (result, &error);
...
thumbnail_opened_cb (GObject *source_object, GAsyncResult *result, ThumbnailData *data)
{
	...
	idol_gdk_pixbuf_new_from_stream_at_scale_async (G_INPUT_STREAM (input_stream), THUMBNAIL_WIDTH, -1, TRUE,
...
query_finished_cb (GObject *source_object, GAsyncResult *result, QueryData *data)
{
	...
	if (g_error_matches (error, GDATA_SERVICE_ERROR, GDATA_SERVICE_ERROR_PROTOCOL_ERROR) == TRUE) {
		...
		idol_interface_error (_("Error Searching for Videos"),
		...
		idol_interface_error (_("Error Searching for Videos"), error->message, window);
	}
...
static void
cancellable_notify_cb (IdolYouTubePlugin *self, GCancellable *old_cancellable)
{
	guint i;
...
static void
set_current_operation (IdolYouTubePlugin *self, guint tree_view, GCancellable *cancellable)
{
...
static void
execute_query (IdolYouTubePlugin *self, guint tree_view, gboolean clear_tree_view)
{
	QueryData *data;
...
void
search_button_clicked_cb (GtkButton *button, IdolYouTubePlugin *self)
{
	...
	if (self->query[SEARCH_TREE_VIEW] == NULL) {
		...
		self->service = gdata_youtube_service_new (DEVELOPER_KEY, NULL);
...
void
cancel_button_clicked_cb (GtkButton *button, IdolYouTubePlugin *self)
{
...
void
search_entry_activate_cb (GtkEntry *entry, IdolYouTubePlugin *self)
{
	search_button_clicked_cb (self->search_button, self);
}

static void
load_related_videos (IdolYouTubePlugin *self)
{
...
void
notebook_switch_page_cb (GtkNotebook *notebook, gpointer *page, guint page_num, IdolYouTubePlugin *self)
{
...
void
open_in_web_browser_activate_cb (GtkAction *action, IdolYouTubePlugin *self)
{
	...
	for (path = paths; path != NULL; path = path->next) {
		...
		if (gtk_show_uri (gtk_widget_get_screen (GTK_WIDGET (self->bvw)), gdata_link_get_uri (link), GDK_CURRENT_TIME, &error) == FALSE) {
			GtkWindow *window = idol_get_main_window (self->idol);
			idol_interface_error (_("Error Opening Video in Web Browser"), error->message, window);
			g_object_unref (window);
			g_error_free (error);
		}
	}
...
void
value_changed_cb (GtkAdjustment *adjustment, IdolYouTubePlugin *self)
{
...
gboolean
button_press_event_cb (GtkWidget *widget, GdkEventButton *event, IdolYouTubePlugin *self)
{
...
gboolean
button_release_event_cb (GtkWidget *widget, GdkEventButton *event, IdolYouTubePlugin *self)
{
...
gboolean
starting_video_cb (IdolVideoList *video_list, GtkTreePath *path, IdolYouTubePlugin *self)
{
	GtkTreeIter iter;
