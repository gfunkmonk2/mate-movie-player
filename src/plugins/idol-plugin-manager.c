...
#define IDOL_PLUGIN_MANAGER_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), IDOL_TYPE_PLUGIN_MANAGER, IdolPluginManagerPrivate))

struct _IdolPluginManagerPrivate
{
	GList		*plugins;
	GtkWidget	*tree;
...
G_DEFINE_TYPE(IdolPluginManager, idol_plugin_manager, GTK_TYPE_VBOX)

static void idol_plugin_manager_finalize (GObject *o);
static IdolPluginInfo *plugin_manager_get_selected_plugin (IdolPluginManager *pm);
static void plugin_manager_toggle_active (GtkTreeIter *iter, GtkTreeModel *model, IdolPluginManager *pm);
static void plugin_manager_toggle_all (IdolPluginManager *pm);

/* Callback functions for GtkBuilder */
G_MODULE_EXPORT void configure_button_cb (GtkWidget *button, IdolPluginManager *pm);

static void
idol_plugin_manager_class_init (IdolPluginManagerClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	object_class->finalize = idol_plugin_manager_finalize;

	g_type_class_add_private (object_class, sizeof (IdolPluginManagerPrivate));
}

void
configure_button_cb (GtkWidget          *button,
		     IdolPluginManager *pm)
{
	IdolPluginInfo *info;
	GtkWindow *toplevel;
	...
	idol_plugins_engine_configure_plugin (info, toplevel);
}
			     ...
			     gpointer           data)
{
	...
	g_return_if_fail (idol_plugins_engine_get_plugin_name (info) != NULL);

	g_object_set (G_OBJECT (cell),
		      "text",
		      idol_plugins_engine_get_plugin_name (info),
		      NULL);
}
		   ...
		   gpointer     data)
{
	...
	gtk_label_set_text (GTK_LABEL (pm->priv->description_text),
			    idol_plugins_engine_get_plugin_description (info));
	gtk_label_set_text (GTK_LABEL (pm->priv->copyright_text),
			    idol_plugins_engine_get_plugin_copyright (info));
	gtk_label_set_text (GTK_LABEL (pm->priv->site_text),
			    idol_plugins_engine_get_plugin_website (info));

	string = g_strjoinv ("\n", (gchar**)idol_plugins_engine_get_plugin_authors (info));
	gtk_label_set_text (GTK_LABEL (pm->priv->authors_text), string);
	g_free (string);

	icon = idol_plugins_engine_get_plugin_icon (info);
	if (icon != NULL) {
	...
	gtk_widget_set_sensitive (GTK_WIDGET (pm->priv->configure_button),
				  (info != NULL) &&
				   idol_plugins_engine_plugin_is_configurable (info));
}
...
column_clicked_cb (GtkTreeViewColumn *tree_column,
		   gpointer           data)
{
	IdolPluginManager *pm = IDOL_PLUGIN_MANAGER (data);

	plugin_manager_toggle_all (pm);
}

static void
plugin_manager_populate_lists (IdolPluginManager *pm)
{
	GtkTreeModel *model;
	GtkTreeIter iter;
	GList *p;
	...
	for (p = pm->priv->plugins; p != NULL; p = g_list_next (p)) {
		...
		gtk_list_store_set (GTK_LIST_STORE (pm->priv->plugin_model), &iter,
				    ACTIVE_COLUMN, idol_plugins_engine_plugin_is_active (info),
				    VISIBLE_COLUMN, idol_plugins_engine_plugin_is_visible (info),
				    INFO_COLUMN, info,
				    -1);
	}
			   ...
			   IdolPluginManager *pm)
{
	...
	if (active) {
		/* activate the plugin */
		if (!idol_plugins_engine_activate_plugin (info)) {
			active ^= 1;
		}
	} else {
		/* deactivate the plugin */
		if (!idol_plugins_engine_deactivate_plugin (info)) {
			active ^= 1;
		}
	}
	...
	gtk_list_store_set (GTK_LIST_STORE (pm->priv->plugin_model),
			    &child_iter,
			    ACTIVE_COLUMN,
			    idol_plugins_engine_plugin_is_active (info),
			    -1);
...
plugin_manager_get_selected_plugin (IdolPluginManager *pm)
{
	IdolPluginInfo *info = NULL;
	GtkTreeModel *model;
	GtkTreeIter iter;
	GtkTreeSelection *selection;
...
static void
plugin_manager_toggle_all (IdolPluginManager *pm)
{
	GtkTreeModel *model;
	GtkTreeIter iter;
		...
		gpointer      data)
{
	...
	normalized_string = g_utf8_normalize (idol_plugins_engine_get_plugin_name (info), -1, G_NORMALIZE_ALL);
...
static void
plugin_manager_construct_tree (IdolPluginManager *pm)
{
	GtkTreeViewColumn *column;
	GtkCellRenderer *cell;
	GtkTreeModel *filter;
...
{
	IdolPluginInfo *lhs = (IdolPluginInfo*)a;
	IdolPluginInfo *rhs = (IdolPluginInfo*)b;
	return strcmp (idol_plugins_engine_get_plugin_name (lhs),
		       idol_plugins_engine_get_plugin_name (rhs));
}

static void
idol_plugin_manager_init (IdolPluginManager *pm)
{
	GtkBuilder *xml;
	GtkWidget *plugins_window;

	pm->priv = IDOL_PLUGIN_MANAGER_GET_PRIVATE (pm);

	xml = idol_interface_load ("plugins.ui", TRUE, NULL, pm);
	...
	pm->priv->plugins = idol_plugins_engine_get_plugins_list ();
...
GtkWidget *
idol_plugin_manager_new (void)
{
	return g_object_new (IDOL_TYPE_PLUGIN_MANAGER, 0);
}

static void
idol_plugin_manager_finalize (GObject *o)
{
	IdolPluginManager *pm = IDOL_PLUGIN_MANAGER (o);

	g_list_free (pm->priv->plugins);

	G_OBJECT_CLASS (idol_plugin_manager_parent_class)->finalize (o);
}
