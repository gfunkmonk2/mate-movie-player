...
#define SAVE_POSITION_FILE_ATTRIBUTE "metadata::idol::position"
...
gboolean
idol_playing_dvd (const char *uri)
{
	if (uri == NULL)
		return FALSE;
...
static void
idol_ensure_dir (const char *path)
{
...
idol_dot_dir (void)
{
	static char *idol_dir = NULL;

	if (idol_dir != NULL) {
		idol_ensure_dir (idol_dir);
		return idol_dir;
	}

	idol_dir = g_build_filename (g_get_user_config_dir (),
				      "idol",
				      NULL);

	idol_ensure_dir (idol_dir);

	return (const char *)idol_dir;
}

const char *
idol_data_dot_dir (void)
{
	static char *idol_dir = NULL;
	...
	idol_dir = g_build_filename (g_get_user_data_dir (),
				      "idol",
				      NULL);
...
static GMount *
idol_get_mount_for_uri (const char *path)
{
	GMount *mount;
	GFile *file;
...
static GMount *
idol_get_mount_for_dvd (const char *uri)
{
	GMount *mount;
	char *path;
	...
	if (g_str_has_prefix (path, "/dev/")) {
		...
		mount = idol_get_mount_for_uri (path);
		g_free (path);
	}
...
static char *
idol_get_mountpoint_for_vcd (const char *uri)
{
	return NULL;
}

GMount *
idol_get_mount_for_media (const char *uri)
{
	GMount *ret;
	char *mount_path;
	...
	if (g_str_has_prefix (uri, "dvd://") != FALSE)
		return idol_get_mount_for_dvd (uri);
	else if (g_str_has_prefix (uri, "vcd:") != FALSE)
		mount_path = idol_get_mountpoint_for_vcd (uri);
	...
	ret = idol_get_mount_for_uri (mount_path);
	g_free (mount_path);
...
gboolean
idol_is_special_mrl (const char *uri)
{
	GMount *mount;
	...
	mount = idol_get_mount_for_media (uri);
	if (mount != NULL)
		g_object_unref (mount);
...
gboolean
idol_is_block_device (const char *uri)
{
	struct stat buf;
	char *local;
...
char *
idol_create_full_path (const char *path)
{
	GFile *file;
	char *retval;
	...
	if (strstr (path, "://") != NULL)
		return NULL;
	if (idol_is_special_mrl (path) != FALSE)
		return NULL;
...
static void
idol_action_on_unmount (GVolumeMonitor *volume_monitor,
			 GMount *mount,
			 Idol *idol)
{
	idol_playlist_clear_with_g_mount (idol->playlist, mount);
}

void
idol_setup_file_monitoring (Idol *idol)
{
	idol->monitor = g_volume_monitor_get ();

	g_signal_connect (G_OBJECT (idol->monitor),
			  "mount-pre-unmount",
			  G_CALLBACK (idol_action_on_unmount),
			  idol);
}
...
gboolean
idol_uri_is_subtitle (const char *uri)
{
	guint len, i;
...
idol_uri_exists (const char *uri)
{
...
static char *
idol_uri_get_subtitle_for_uri (const char *uri)
{
	char *subtitle;
	guint len, i;
	gint suffix;
...
static char *
idol_uri_get_subtitle_in_subdir (GFile *file, const char *subdir)
{
	...
	subtitle = idol_uri_get_subtitle_for_uri (full_path_str);
...
static char *
idol_uri_get_cached_subtitle_for_uri (const char *uri)
{
	...
	ret = idol_uri_get_subtitle_for_uri (fake_uri);
	g_free (fake_uri);
...
char *
idol_uri_get_subtitle_uri (const char *uri)
{
	GFile *file;
	char *subtitle;
	...
	subtitle = idol_uri_get_cached_subtitle_for_uri (uri);
	if (subtitle != NULL) {
		g_object_unref (file);
		return subtitle;
	}
	...
	subtitle = idol_uri_get_subtitle_for_uri (uri);
	if (subtitle != NULL) {
		g_object_unref (file);
		return subtitle;
	}

	subtitle = idol_uri_get_subtitle_in_subdir (file, "subtitles");
	g_object_unref (file);
...
char *
idol_uri_escape_for_display (const char *uri)
{
	GFile *file;
	char *disp;
...
void
idol_destroy_file_filters (void)
{
	if (filter_all != NULL) {
...
static void
idol_add_default_dirs (GtkFileChooser *dialog)
{
	guint i;
...
char *
idol_add_subtitle (GtkWindow *parent, const char *uri)
{
	GtkWidget *fs;
	MateConfClient *conf;
	char *new_path;
	...
	new_path = mateconf_client_get_string (conf, "/apps/idol/open_path", NULL);
	...
	idol_add_default_dirs (GTK_FILE_CHOOSER (fs));
...
GSList *
idol_add_files (GtkWindow *parent, const char *path)
{
	GtkWidget *fs;
	int response;
	GSList *filenames;
	...
	if (mrl != NULL) {
		...
		mateconf_client_set_string (conf, "/apps/idol/open_path",
				new_path, NULL);
		g_free (new_path);
	}
...
idol_save_position (Idol *idol)
{
	...
	if (idol->remember_position == FALSE)
		return;
	if (idol->mrl == NULL)
		return;

	stream_length = bacon_video_widget_get_stream_length (idol->bvw);
	position = bacon_video_widget_get_current_time (idol->bvw);

	file = g_file_new_for_uri (idol->mrl);
...
void
idol_try_restore_position (Idol *idol, const char *mrl)
{
	GFile *file;
	GFileInfo *file_info;
	const char *seek_str;
	...
	if (seek_str != NULL)
		idol->seek_to = g_ascii_strtoull (seek_str, NULL, 0);
