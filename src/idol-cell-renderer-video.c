...
/**
 * SECTION:idol-cell-renderer-video
 * @short_description: a #GtkCellRenderer widget for listing videos
 * @stability: Unstable
 * @include: idol-cell-renderer-video.h
 * @see_also: #IdolVideoList
 *
 * #IdolCellRendererVideo is a #GtkCellRenderer for rendering video thumbnails, typically in a #IdolVideoList.
...
struct _IdolCellRendererVideoPrivate {
	gchar *title;
	GdkPixbuf *thumbnail;
...
#define IDOL_CELL_RENDERER_VIDEO_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE ((obj), IDOL_TYPE_CELL_RENDERER_VIDEO, IdolCellRendererVideoPrivate))
...
static void idol_cell_renderer_video_set_property (GObject *object, guint property_id, const GValue *value, GParamSpec *pspec);
static void idol_cell_renderer_video_get_property (GObject *object, guint property_id, GValue *value, GParamSpec *pspec);
static void idol_cell_renderer_video_dispose (GObject *object);
static void idol_cell_renderer_video_finalize (GObject *object);
static void idol_cell_renderer_video_get_size (GtkCellRenderer *cell, GtkWidget *widget, GdkRectangle *cell_area, gint *x_offset, gint *y_offset, gint *width, gint *height);
static void idol_cell_renderer_video_render (GtkCellRenderer *cell, GdkDrawable *window, GtkWidget *widget, GdkRectangle *background_area, GdkRectangle *cell_area, GdkRectangle *expose_area, GtkCellRendererState flags);

G_DEFINE_TYPE (IdolCellRendererVideo, idol_cell_renderer_video, GTK_TYPE_CELL_RENDERER)

/**
 * idol_cell_renderer_video_new:
 * @use_placeholder: if %TRUE, use a placeholder thumbnail
 *
 * Creates a new #IdolCellRendererVideo with its #IdolCellRendererVideo:use-placeholder
 ...
 *
 * Return value: a new #IdolCellRendererVideo
 **/
IdolCellRendererVideo *
idol_cell_renderer_video_new (gboolean use_placeholder)
{
	return g_object_new (IDOL_TYPE_CELL_RENDERER_VIDEO,
				"use-placeholder", use_placeholder,
				NULL); 
}

static void
idol_cell_renderer_video_class_init (IdolCellRendererVideoClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);
	GtkCellRendererClass *renderer_class = GTK_CELL_RENDERER_CLASS (klass);

	g_type_class_add_private (klass, sizeof (IdolCellRendererVideoPrivate));

	object_class->set_property = idol_cell_renderer_video_set_property;
	object_class->get_property = idol_cell_renderer_video_get_property;
	object_class->dispose = idol_cell_renderer_video_dispose;
	object_class->finalize = idol_cell_renderer_video_finalize;
	renderer_class->get_size = idol_cell_renderer_video_get_size;
	renderer_class->render = idol_cell_renderer_video_render;

	/**
	 * IdolCellRendererVideo:thumbnail:
	 *
	...
	/**
	 * IdolCellRendererVideo:alignment:
	 *
	...
	/**
	 * IdolCellRendererVideo:use-placeholder:
	 *
	 * If %TRUE, a placeholder image should be used for the video thumbnail if a #IdolCellRendererVideo:thumbnail isn't provided.
	 **/
...
static void
idol_cell_renderer_video_init (IdolCellRendererVideo *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, IDOL_TYPE_CELL_RENDERER_VIDEO, IdolCellRendererVideoPrivate);
...
static void
idol_cell_renderer_video_dispose (GObject *object)
{
	IdolCellRendererVideo *self = IDOL_CELL_RENDERER_VIDEO (object);
	...
	G_OBJECT_CLASS (idol_cell_renderer_video_parent_class)->dispose (object);
}

static void
idol_cell_renderer_video_finalize (GObject *object)
{
	IdolCellRendererVideo *self = IDOL_CELL_RENDERER_VIDEO (object);
	...
	/* Chain up to the parent class */
	G_OBJECT_CLASS (idol_cell_renderer_video_parent_class)->finalize (object);
}

static void
idol_cell_renderer_video_set_property (GObject *object, guint property_id, const GValue *value, GParamSpec *pspec)
{
	IdolCellRendererVideoPrivate *priv = IDOL_CELL_RENDERER_VIDEO_GET_PRIVATE (object);
...
static void
idol_cell_renderer_video_get_property (GObject *object, guint property_id, GValue *value, GParamSpec *pspec)
{
	IdolCellRendererVideoPrivate *priv = IDOL_CELL_RENDERER_VIDEO_GET_PRIVATE (object);
...
{
	IdolCellRendererVideoPrivate *priv = IDOL_CELL_RENDERER_VIDEO (cell)->priv;
	GtkStyle *style;
...
static void
idol_cell_renderer_video_get_size (GtkCellRenderer *cell,
				    GtkWidget *widget,
				    GdkRectangle *cell_area,
				    gint *x_offset,
				    gint *y_offset,
...
static void
idol_cell_renderer_video_render (GtkCellRenderer *cell,
				  GdkDrawable *window,
				  GtkWidget *widget,
