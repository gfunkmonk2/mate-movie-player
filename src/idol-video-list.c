...
/**
 * SECTION:idol-video-list
 * @short_description: a #GtkTreeView widget for listing videos
 * @stability: Unstable
 * @include: idol-video-list.h
 * @see_also: #IdolCellRendererVideo
 *
 * #IdolVideoList is a #GtkTreeView designed for listing videos by their thumbnails using #IdolCellRendererVideo.
...
#include "idolvideolist-marshal.h"
#include "idol-interface.h"

struct _IdolVideoListPrivate {
	gint tooltip_column;
	gint mrl_column;
	Idol *idol;
	GtkBuilder *xml;
...
static void idol_video_list_dispose (GObject *object);
static void idol_video_list_set_property (GObject *object, guint property_id, const GValue *value, GParamSpec *pspec);
static void idol_video_list_get_property (GObject *object, guint property_id, GValue *value, GParamSpec *pspec);
...
static gboolean default_starting_video_cb (IdolVideoList *video_list, GtkTreePath *path);
static gboolean show_popup_menu (IdolVideoList *self, GdkEventButton *event);
static gboolean button_pressed_cb (GtkTreeView *tree_view, GdkEventButton *event, IdolVideoList *video_list);
static gboolean popup_menu_cb (GtkTreeView *tree_view, IdolVideoList *video_list);

/* Callback functions for GtkBuilder */
G_MODULE_EXPORT void add_to_playlist_action_callback (GtkAction *action, IdolVideoList *self);
G_MODULE_EXPORT void copy_location_action_callback (GtkAction *action, IdolVideoList *self);

static gint idol_video_list_table_signals[LAST_SIGNAL] = { 0 };

G_DEFINE_TYPE (IdolVideoList, idol_video_list, GTK_TYPE_TREE_VIEW)

/**
 * idol_video_list_new:
 *
 * Creates a new #IdolVideoList with default properties.
 *
 * Return value: a new #IdolVideoList
 **/
IdolVideoList *
idol_video_list_new (void)
{
	IdolVideoList *video_list;

	video_list = IDOL_VIDEO_LIST (g_object_new (IDOL_TYPE_VIDEO_LIST, NULL));
...
static void
idol_video_list_class_init (IdolVideoListClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	g_type_class_add_private (klass, sizeof (IdolVideoListPrivate));

	object_class->set_property = idol_video_list_set_property;
	object_class->get_property = idol_video_list_get_property;
	object_class->dispose = idol_video_list_dispose;
	klass->starting_video = default_starting_video_cb;

	/**
	 * IdolVideoList:tooltip-column:
	 *
	 ...
	 * IdolVideoList:idol:
	 *
	 * A #IdolObject for integration purposes.
	 **/
	/* FIXME: Is there no better way to do this? */
	g_object_class_install_property (object_class, PROP_IDOL,
				g_param_spec_object ("idol", NULL, NULL,
	 ...
	 *
	 * The #IdolVideoList::starting-video signal is emitted when a video row is activated, just before the video is
	 ...
	 */
	idol_video_list_table_signals[STARTING_VIDEO] = g_signal_new ("starting-video",
				G_TYPE_FROM_CLASS (object_class),
				G_SIGNAL_RUN_LAST,
				G_STRUCT_OFFSET (IdolVideoListClass, starting_video),
				NULL, NULL,
				idolvideolist_marshal_BOOLEAN__BOXED,
				G_TYPE_BOOLEAN, 1,
				GTK_TYPE_TREE_PATH);
}

static void
idol_video_list_init (IdolVideoList *self)
{
	GtkTreeSelection *selection;

	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, IDOL_TYPE_VIDEO_LIST, IdolVideoListPrivate);
	self->priv->idol = NULL;
	...
	self->priv->ui_manager = GTK_UI_MANAGER (gtk_builder_get_object (self->priv->xml, "idol-video-list-ui-manager"));
...
static void
idol_video_list_dispose (GObject *object)
{
	IdolVideoListPrivate *priv = IDOL_VIDEO_LIST_GET_PRIVATE (object);

	if (priv->idol != NULL) {
		g_object_unref (priv->idol);
		priv->idol = NULL;
	...
	G_OBJECT_CLASS (idol_video_list_parent_class)->dispose (object);
}

static void
idol_video_list_set_property (GObject *object, guint property_id, const GValue *value, GParamSpec *pspec)
{
	IdolVideoListPrivate *priv = IDOL_VIDEO_LIST_GET_PRIVATE (object);
...
static void
idol_video_list_get_property (GObject *object, guint property_id, GValue *value, GParamSpec *pspec)
{
	IdolVideoListPrivate *priv = IDOL_VIDEO_LIST_GET_PRIVATE (object);
...
{
	IdolVideoList *self = IDOL_VIDEO_LIST (widget);
	GtkTreeIter iter;
	gchar *tooltip_text;
	gchar *mrl_text;
	gchar *final_text;
...
row_activated_cb (GtkTreeView *tree_view, GtkTreePath *path, GtkTreeViewColumn *column, gpointer user_data)
{
	...
	IdolVideoList *self = IDOL_VIDEO_LIST (tree_view);
	GtkTreeModel *model = gtk_tree_view_get_model (tree_view);
	...
	g_signal_emit (tree_view, idol_video_list_table_signals[STARTING_VIDEO], 0,
		       path,
		       &play_video);
	...
	if (mrl != NULL)
		idol_add_to_playlist_and_play (self->priv->idol, mrl, display_name, FALSE);
...
static gboolean
default_starting_video_cb (IdolVideoList *video_list, GtkTreePath *path)
{
...
static gboolean
show_popup_menu (IdolVideoList *self, GdkEventButton *event)
{
	guint button = 0;
	guint32 _time;
	GtkTreePath *path;
	...
	menu = gtk_ui_manager_get_widget (self->priv->ui_manager, "/idol-video-list-popup");
...
static gboolean
button_pressed_cb (GtkTreeView *tree_view, GdkEventButton *event, IdolVideoList *video_list)
{
...
static gboolean
popup_menu_cb (GtkTreeView *tree_view, IdolVideoList *video_list)
{
	return show_popup_menu (video_list, NULL);
}

void
add_to_playlist_action_callback (GtkAction *action, IdolVideoList *self)
{
	GtkTreePath *path;
	GtkTreeIter iter;
	...
	IdolPlaylist *playlist = self->priv->idol->playlist;
...
void
copy_location_action_callback (GtkAction *action, IdolVideoList *self)
{
	GList *l;
	GtkTreePath *path;
	GtkClipboard *clip;
 ...
 **/
GtkUIManager *
idol_video_list_get_ui_manager (IdolVideoList *self)
{
