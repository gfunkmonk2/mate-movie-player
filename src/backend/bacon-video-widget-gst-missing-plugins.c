...
GST_DEBUG_CATEGORY_EXTERN (_idol_gst_debug_cat);
...
IdolCodecInstallContext;

#ifdef GDK_WINDOWING_X11
/* Adapted from idol-interface.c */
...
static void
bacon_video_widget_gst_codec_install_context_free (IdolCodecInstallContext *ctx)
{
...
{
	IdolCodecInstallContext *ctx = (IdolCodecInstallContext *) user_data;
	gchar **p;
						 ...
						 gpointer user_data)
{
	...
	ctx = g_new0 (IdolCodecInstallContext, 1);
	 ...
	 * be able to use idol's controls while the wizard is running */
	if (playing)
