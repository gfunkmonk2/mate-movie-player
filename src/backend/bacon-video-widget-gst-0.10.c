 ...
 * backend, and abstracts away the differences to provide a simple interface to the functionality required by Idol. It handles all the low-level
 * audio and video work for Idol (or passes the work off to the backend).
 **/
...
GST_DEBUG_CATEGORY (_idol_gst_debug_cat);
#define GST_CAT_DEFAULT _idol_gst_debug_cat
...
bacon_video_widget_realize (GtkWidget * widget)
{
  ...
  idol_widget_set_preferred_size (widget, w, h);
 ...
 * from the bus message callback (via EOS => idol => close => wait for READY)
...
timed_out:
  /* it's taking a long time to open -- just tell idol it was ok, this allows
                         ...
                         const gchar * mrl, const gchar *subtitle_uri, GError ** error)
{
  ...
  if (ret) {
    ...
    if (bvw_check_missing_plugins_error (bvw, err_msg)) {
      /* idol will try to start playing, so ignore all messages on the bus */
                                    ...
                                    gboolean show_cursor)
{
  ...
  if (show_cursor == FALSE) {
    idol_gdk_window_set_invisible_cursor (window);
  } else {
...
bacon_video_widget_set_scale_ratio (BaconVideoWidget * bvw, gfloat ratio)
{
  ...
  if (ratio == 0.0) {
    if (idol_ratio_fits_screen (bvw->priv->video_window, w, h, 2.0))
      ratio = 2.0;
    else if (idol_ratio_fits_screen (bvw->priv->video_window, w, h, 1.0))
      ratio = 1.0;
    else if (idol_ratio_fits_screen (bvw->priv->video_window, w, h, 0.5))
      ratio = 0.5;
    else
      return;
  } else {
    if (!idol_ratio_fits_screen (bvw->priv->video_window, w, h, ratio)) {
  ...
  idol_widget_set_preferred_size (GTK_WIDGET (bvw), w, h);
}
 ...
 * DVD drive.
 *
 * @type can be any value from #IdolDiscMediaType, but a %BVW_ERROR_INVALID_LOCATION error
 * will be returned if @type is %MEDIA_TYPE_CDDA, as CDDA support has been removed from
 * Idol (and hence #BaconVideoWidget).
 *
                        ...
                        BvwUseType type, GError ** error)
{
  ...
  if (_idol_gst_debug_cat == NULL) {
    GST_DEBUG_CATEGORY_INIT (_idol_gst_debug_cat, "idol", 0,
        "Idol GStreamer Backend");
  }
#endif
