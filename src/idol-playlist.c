...
#include "idolplaylist-marshal.h"
...
static void ensure_shuffled (IdolPlaylist *playlist);
static gboolean idol_playlist_add_one_mrl (IdolPlaylist *playlist, const char *mrl, const char *display_name);

typedef gboolean (*ClearComparisonFunc) (IdolPlaylist *playlist, GtkTreeIter *iter, gconstpointer data);

static void idol_playlist_clear_with_compare (IdolPlaylist *playlist,
					       ClearComparisonFunc func,
					       gconstpointer data);

/* Callback function for GtkBuilder */
G_MODULE_EXPORT void idol_playlist_save_files (GtkWidget *widget, IdolPlaylist *playlist);
G_MODULE_EXPORT void idol_playlist_add_files (GtkWidget *widget, IdolPlaylist *playlist);
G_MODULE_EXPORT void playlist_remove_button_clicked (GtkWidget *button, IdolPlaylist *playlist);
G_MODULE_EXPORT void idol_playlist_up_files (GtkWidget *widget, IdolPlaylist *playlist);
G_MODULE_EXPORT void idol_playlist_down_files (GtkWidget *widget, IdolPlaylist *playlist);
G_MODULE_EXPORT void playlist_copy_location_action_callback (GtkAction *action, IdolPlaylist *playlist);
G_MODULE_EXPORT void playlist_select_subtitle_action_callback (GtkAction *action, IdolPlaylist *playlist);
G_MODULE_EXPORT void playlist_remove_action_callback (GtkAction *action, IdolPlaylist *playlist);
...
static const PlaylistSaveType save_types [] = {
	...
	{ N_("MP3 ShoutCast playlist"), "pls", IDOL_PL_PARSER_PLS },
	{ N_("MP3 audio (streamed)"), "m3u", IDOL_PL_PARSER_M3U },
	{ N_("MP3 audio (streamed, DOS format)"), "m3u", IDOL_PL_PARSER_M3U_DOS },
	{ N_("XML Shareable Playlist"), "xspf", IDOL_PL_PARSER_XSPF }
};

static int idol_playlist_table_signals[LAST_SIGNAL];
...
static void init_treeview (GtkWidget *treeview, IdolPlaylist *playlist);

#define idol_playlist_unset_playing(x) idol_playlist_set_playing(x, IDOL_PLAYLIST_STATUS_NONE)

G_DEFINE_TYPE (IdolPlaylist, idol_playlist, GTK_TYPE_VBOX)

/* Helper functions */
static gboolean
idol_playlist_gtk_tree_model_iter_previous (GtkTreeModel *tree_model,
		GtkTreeIter *iter)
{
	GtkTreePath *path;
	gboolean ret;
...
static gboolean
idol_playlist_gtk_tree_path_equals (GtkTreePath *path1, GtkTreePath *path2)
{
	char *str1, *str2;
	gboolean retval;
...
static GtkWindow *
idol_playlist_get_toplevel (IdolPlaylist *playlist)
{
	return GTK_WINDOW (gtk_widget_get_toplevel (GTK_WIDGET (playlist)));
}

static void
set_waiting_cursor (IdolPlaylist *playlist)
{
	idol_gdk_window_set_waiting_cursor (gtk_widget_get_window (GTK_WIDGET (idol_playlist_get_toplevel (playlist))));
	playlist->priv->cursor_ref++;
}

static void
unset_waiting_cursor (IdolPlaylist *playlist)
{
	if (--playlist->priv->cursor_ref < 1)
		gdk_window_set_cursor (gtk_widget_get_window (GTK_WIDGET (idol_playlist_get_toplevel (playlist))), NULL);
}

static void
idol_playlist_error (char *title, char *reason, IdolPlaylist *playlist)
{
	GtkWidget *error_dialog;

	error_dialog =
		gtk_message_dialog_new (idol_playlist_get_toplevel (playlist),
				GTK_DIALOG_MODAL,
				GTK_MESSAGE_ERROR,
				GTK_BUTTONS_OK,
				"%s", title);
...
void
idol_playlist_select_subtitle_dialog(IdolPlaylist *playlist, IdolPlaylistSelectDialog mode)
{
	char *subtitle, *current, *uri;
	GFile *file, *dir;
	IdolPlaylistStatus playing;
	GtkTreeIter iter;

	if (mode == IDOL_PLAYLIST_DIALOG_PLAYING) {
		/* Set subtitle file for the currently playing movie */
		gtk_tree_model_get_iter (playlist->priv->model, &iter, playlist->priv->current);
	} else if (mode == IDOL_PLAYLIST_DIALOG_SELECTED) {
	...
	subtitle = idol_add_subtitle (idol_playlist_get_toplevel (playlist), uri);
	g_free (uri);
	...
	if (playing != IDOL_PLAYLIST_STATUS_NONE) {
		g_signal_emit (G_OBJECT (playlist),
			       idol_playlist_table_signals[SUBTITLE_CHANGED], 0,
			       NULL);
	}
...
void
idol_playlist_set_current_subtitle (IdolPlaylist *playlist, const char *subtitle_uri)
{
	GtkTreeIter iter;
 ...
 */
static char *
idol_playlist_mrl_to_title (const gchar *mrl)
{
	GFile *file;
...
static void
idol_playlist_update_save_button (IdolPlaylist *playlist)
{
	gboolean state;
...
static gboolean
idol_playlist_save_iter_foreach (GtkTreeModel *model,
				  GtkTreePath  *path,
				  GtkTreeIter  *iter,
				  gpointer      user_data)
{
	IdolPlPlaylist *playlist = user_data;
	...
	idol_pl_playlist_set (playlist, &pl_iter,
			       ...
			       IDOL_PL_PARSER_FIELD_TITLE, (custom_title) ? title : NULL,
			       NULL);
...
void
idol_playlist_save_current_playlist (IdolPlaylist *playlist, const char *output)
{
	idol_playlist_save_current_playlist_ext (playlist, output, IDOL_PL_PARSER_PLS);
}

void
idol_playlist_save_current_playlist_ext (IdolPlaylist *playlist, const char *output, IdolPlParserType type)
{
	IdolPlPlaylist *pl_playlist;
	GError *error = NULL;
	GFile *output_file;
	gboolean retval;

	pl_playlist = idol_pl_playlist_new ();
	output_file = g_file_new_for_commandline_arg (output);

	gtk_tree_model_foreach (playlist->priv->model,
				idol_playlist_save_iter_foreach,
				pl_playlist);

	retval = idol_pl_parser_save (playlist->priv->parser,
				       pl_playlist,
				       output_file,
				       NULL, type, &error);
	...
	if (retval == FALSE)
	{
		idol_playlist_error (_("Could not save the playlist"),
	 ...
	 IdolPlaylist    *playlist)
{
	...
	for (p = file_list; p != NULL; p = p->next) {
		...
		filename = idol_create_full_path (p->data);
		if (filename == NULL)
...
void
playlist_select_subtitle_action_callback (GtkAction *action, IdolPlaylist *playlist)
{
	idol_playlist_select_subtitle_dialog (playlist, IDOL_PLAYLIST_DIALOG_SELECTED);
}

void
playlist_copy_location_action_callback (GtkAction *action, IdolPlaylist *playlist)
{
	GList *l;
	GtkClipboard *clip;
	char *url;
...
static gboolean
playlist_show_popup_menu (IdolPlaylist *playlist, GdkEventButton *event)
{
	guint button = 0;
	guint32 _time;
	GtkTreePath *path;
	...
	menu = gtk_ui_manager_get_widget (playlist->priv->ui_manager, "/idol-playlist-popup");
...
static gboolean
playlist_treeview_popup_menu (GtkTreeView *treeview, IdolPlaylist *playlist)
{
	return playlist_show_popup_menu (playlist, NULL);
}

static void
idol_playlist_set_reorderable (IdolPlaylist *playlist, gboolean set)
{
	guint num_items, i;
	...
	for (i = 0; i < num_items; i++)
	{
		...
		if (gtk_tree_path_compare (path, playlist->priv->current) == 0) {
			...
			g_signal_emit (G_OBJECT (playlist),
					idol_playlist_table_signals[CHANGED],
					0, NULL);
		}
...
{ 
	IdolPlaylist *playlist = (IdolPlaylist *)data;
	...
	idol_playlist_set_reorderable (playlist, TRUE);
...
button_release_cb (GtkWidget *treeview, GdkEventButton *event, gpointer data)
{
	...
	if (!playlist->priv->drag_started && playlist->priv->drop_disabled)
	{
		...
		idol_playlist_set_reorderable (playlist, FALSE);
...
static void
selection_changed (GtkTreeSelection *treeselection, IdolPlaylist *playlist)
{
 ...
 * current item */
static gboolean
update_current_from_playlist (IdolPlaylist *playlist)
{
	int indice;
...
static void
idol_playlist_foreach_selected (GtkTreeModel *model, GtkTreePath *path,
...
static void
idol_playlist_emit_item_removed (IdolPlaylist *playlist,
				  GtkTreeIter   *iter)
{
	gchar *filename = NULL;
	gchar *uri = NULL;
	...
	g_signal_emit (playlist,
		       idol_playlist_table_signals[ITEM_REMOVED],
		       0, filename, uri);
...
static void
playlist_remove_files (IdolPlaylist *playlist)
{
	idol_playlist_clear_with_compare (playlist, NULL, NULL);
}

void
playlist_remove_button_clicked (GtkWidget *button, IdolPlaylist *playlist)
{
	playlist_remove_files (playlist);
}

void
playlist_remove_action_callback (GtkAction *action, IdolPlaylist *playlist)
{
	playlist_remove_files (playlist);
}

static void
idol_playlist_save_playlist (IdolPlaylist *playlist, char *filename, gint active_format)
{
	if (active_format == 0)
		active_format = 1;

	idol_playlist_save_current_playlist_ext (playlist, filename,
...
static void
format_selection_changed (EggFileFormatChooser *chooser, IdolPlaylist *playlist)
{
	guint format;
...
static GtkWidget *
idol_playlist_save_add_format_chooser (GtkFileChooser *fc, IdolPlaylist *playlist)
{
...
void
idol_playlist_save_files (GtkWidget *widget, IdolPlaylist *playlist)
{
	...
	fs = gtk_file_chooser_dialog_new (_("Save Playlist"),
					  idol_playlist_get_toplevel (playlist),
	...
	g_free (filename);
	format_chooser = idol_playlist_save_add_format_chooser (GTK_FILE_CHOOSER (fs), playlist);
	...
	if (response == GTK_RESPONSE_ACCEPT) {
		...
		idol_playlist_save_playlist (playlist, fname, active_format);
		g_free (fname);
	} else {
		playlist->priv->file_chooser = NULL;
		gtk_widget_destroy (fs);
...
static void
idol_playlist_move_files (IdolPlaylist *playlist, gboolean direction_up)
{
	GtkTreeSelection *selection;
	GtkTreeModel *model;
	GtkListStore *store;
...
void
idol_playlist_up_files (GtkWidget *widget, IdolPlaylist *playlist)
{
	idol_playlist_move_files (playlist, TRUE);
}

void
idol_playlist_down_files (GtkWidget *widget, IdolPlaylist *playlist)
{
	idol_playlist_move_files (playlist, FALSE);
}

static int
idol_playlist_key_press (GtkWidget *win, GdkEventKey *event, IdolPlaylist *playlist)
{
...
set_playing_icon (GtkTreeViewColumn *column, GtkCellRenderer *renderer,
		  GtkTreeModel *model, GtkTreeIter *iter, IdolPlaylist *playlist)
{
	IdolPlaylistStatus playing;
	const char *stock_id;
	...
	switch (playing) {
		case IDOL_PLAYLIST_STATUS_PLAYING:
			stock_id = GTK_STOCK_MEDIA_PLAY;
			break;
		case IDOL_PLAYLIST_STATUS_PAUSED:
...
static void
init_columns (GtkTreeView *treeview, IdolPlaylist *playlist)
{
...
treeview_row_changed (GtkTreeView *treeview, GtkTreePath *arg1,
		GtkTreeViewColumn *arg2, IdolPlaylist *playlist)
{
	if (idol_playlist_gtk_tree_path_equals
	    (arg1, playlist->priv->current) != FALSE) {
		g_signal_emit (G_OBJECT (playlist),
				idol_playlist_table_signals[ITEM_ACTIVATED], 0,
				NULL);
		return;
	}

	if (playlist->priv->current != NULL) {
		idol_playlist_unset_playing (playlist);
		gtk_tree_path_free (playlist->priv->current);
	}
...
static void
init_treeview (GtkWidget *treeview, IdolPlaylist *playlist)
{
	GtkTreeSelection *selection;
...
update_repeat_cb (MateConfClient *client, guint cnxn_id,
		MateConfEntry *entry, IdolPlaylist *playlist)
{
	gboolean repeat;
	...
	g_signal_emit (G_OBJECT (playlist),
			...
			NULL);
	g_signal_emit (G_OBJECT (playlist),
			idol_playlist_table_signals[REPEAT_TOGGLED], 0,
			repeat, NULL);
}
...
static void
ensure_shuffled (IdolPlaylist *playlist)
{
	RandomData data;
	GArray *array;
		...
		MateConfEntry *entry, IdolPlaylist *playlist)
{
	...
	g_signal_emit (G_OBJECT (playlist),
			...
			NULL);
	g_signal_emit (G_OBJECT (playlist),
			idol_playlist_table_signals[SHUFFLE_TOGGLED], 0,
			shuffle, NULL);
}
		...
		MateConfEntry *entry, IdolPlaylist *playlist)
{
	...
	idol_playlist_update_save_button (playlist);
}

static void
init_config (IdolPlaylist *playlist)
{
...
static void
idol_playlist_entry_parsed (IdolPlParser *parser,
			     const char *uri,
			     GHashTable *metadata,
			     IdolPlaylist *playlist)
{
	...
	duration = idol_pl_parser_parse_duration
		(g_hash_table_lookup (metadata, IDOL_PL_PARSER_FIELD_DURATION), FALSE);
	if (duration == 0)
		return;
	title = g_hash_table_lookup (metadata, IDOL_PL_PARSER_FIELD_TITLE);
	idol_playlist_add_one_mrl (playlist, uri, title);
}

static gboolean
idol_playlist_compare_with_monitor (IdolPlaylist *playlist, GtkTreeIter *iter, gconstpointer data)
{
...
static void
idol_playlist_file_changed (GFileMonitor *monitor,
			     GFile *file,
			     GFile *other_file,
			     ...
			     IdolPlaylist *playlist)
{
	if (event_type == G_FILE_MONITOR_EVENT_PRE_UNMOUNT ||
		...
		idol_playlist_clear_with_compare (playlist,
						   (ClearComparisonFunc) idol_playlist_compare_with_monitor,
						   monitor);
	}
}

static void
idol_playlist_dispose (GObject *object)
{
	IdolPlaylist *playlist = IDOL_PLAYLIST (object);
	...
	G_OBJECT_CLASS (idol_playlist_parent_class)->dispose (object);
}

static void
idol_playlist_finalize (GObject *object)
{
	IdolPlaylist *playlist = IDOL_PLAYLIST (object);
	...
	G_OBJECT_CLASS (idol_playlist_parent_class)->finalize (object);
}

static void
idol_playlist_init (IdolPlaylist *playlist)
{
	GtkWidget *container;
	GtkBuilder *xml;

	playlist->priv = G_TYPE_INSTANCE_GET_PRIVATE (playlist, IDOL_TYPE_PLAYLIST, IdolPlaylistPrivate);
	playlist->priv->parser = idol_pl_parser_new ();

	idol_pl_parser_add_ignored_scheme (playlist->priv->parser, "dvd:");
	idol_pl_parser_add_ignored_scheme (playlist->priv->parser, "cdda:");
	idol_pl_parser_add_ignored_scheme (playlist->priv->parser, "vcd:");
	idol_pl_parser_add_ignored_scheme (playlist->priv->parser, "cd:");
	idol_pl_parser_add_ignored_scheme (playlist->priv->parser, "dvb:");
	idol_pl_parser_add_ignored_mimetype (playlist->priv->parser, "application/x-trash");

	g_signal_connect (G_OBJECT (playlist->priv->parser),
			"entry-parsed",
			G_CALLBACK (idol_playlist_entry_parsed),
			playlist);

	xml = idol_interface_load ("playlist.ui", TRUE, NULL, playlist);
	...
	playlist->priv->ui_manager = GTK_UI_MANAGER (gtk_builder_get_object (xml, "idol-playlist-ui-manager"));
	...
	g_signal_connect (G_OBJECT (playlist), "key_press_event",
			  G_CALLBACK (idol_playlist_key_press), playlist);
...
GtkWidget*
idol_playlist_new (void)
{
	IdolPlaylist *playlist;

	playlist = IDOL_PLAYLIST (g_object_new (IDOL_TYPE_PLAYLIST, NULL));
	if (playlist->priv->ui_manager == NULL) {
...
static gboolean
idol_playlist_add_one_mrl (IdolPlaylist *playlist,
			    const char *mrl,
			    const char *display_name)
{
	...
	g_return_val_if_fail (IDOL_IS_PLAYLIST (playlist), FALSE);
	g_return_val_if_fail (mrl != NULL, FALSE);

	if (display_name == NULL || *display_name == '\0')
		filename_for_display = idol_playlist_mrl_to_title (mrl);
	else
		filename_for_display = g_strdup (display_name);

	ref = NULL;
	uri = idol_create_full_path (mrl);

	g_debug ("idol_playlist_add_one_mrl (): %s %s %s\n", filename_for_display, uri, display_name);
	...
	if (g_file_is_native (file) != FALSE) {
		...
		g_signal_connect (G_OBJECT (monitor),
				  "changed",
				  G_CALLBACK (idol_playlist_file_changed),
				  playlist);
	} else {
		monitor = NULL;
	}
	...
	gtk_list_store_insert_with_values (store, &iter, pos,
			PLAYING_COL, IDOL_PLAYLIST_STATUS_NONE,
	...
	g_signal_emit (playlist,
		       idol_playlist_table_signals[ITEM_ADDED],
...
static gboolean
handle_parse_result (IdolPlParserResult res, IdolPlaylist *playlist, const gchar *mrl, const gchar *display_name)
{
	if (res == IDOL_PL_PARSER_RESULT_UNHANDLED)
		return idol_playlist_add_one_mrl (playlist, mrl, display_name);
	if (res == IDOL_PL_PARSER_RESULT_ERROR) {
		char *msg;

		msg = g_strdup_printf (_("The playlist '%s' could not be parsed. It might be damaged."), display_name ? display_name : mrl);
		idol_playlist_error (_("Playlist error"), msg, playlist);
		g_free (msg);

		return FALSE;
	}
	if (res == IDOL_PL_PARSER_RESULT_IGNORED)
		return FALSE;
...
static void
add_mrl_cb (IdolPlParser *parser, GAsyncResult *result, AddMrlData *data)
{
	...
	res = idol_pl_parser_parse_finish (parser, result, &error);

	/* Remove the cursor, if one was set */
	if (data->cursor)
		unset_waiting_cursor (data->playlist);

	/* Create an async result which will return the result to the code which called idol_playlist_add_mrl() */
	if (error != NULL)
		async_result = g_simple_async_result_new_from_error (G_OBJECT (data->playlist), data->callback, data->user_data, error);
	else
		async_result = g_simple_async_result_new (G_OBJECT (data->playlist), data->callback, data->user_data, idol_playlist_add_mrl);
	...
	/* Synchronously call the calling code's callback function (i.e. what was passed to idol_playlist_add_mrl()'s @callback parameter)
	 * in the main thread to return the result */
	g_simple_async_result_complete (async_result);
}

void
idol_playlist_add_mrl (IdolPlaylist *playlist, const char *mrl, const char *display_name, gboolean cursor,
                        GCancellable *cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
	 ...
	 * finish the process. */
	idol_pl_parser_parse_async (playlist->priv->parser, mrl, FALSE, cancellable, (GAsyncReadyCallback) add_mrl_cb, data);
}

gboolean
idol_playlist_add_mrl_finish (IdolPlaylist *playlist, GAsyncResult *result)
{
	g_assert (g_simple_async_result_get_source_tag (G_SIMPLE_ASYNC_RESULT (result)) == idol_playlist_add_mrl);

	return g_simple_async_result_get_op_res_gboolean (G_SIMPLE_ASYNC_RESULT (result));
}

gboolean
idol_playlist_add_mrl_sync (IdolPlaylist *playlist, const char *mrl, const char *display_name)
{
	g_return_val_if_fail (mrl != NULL, FALSE);
...
idol_playlist_clear_cb (GtkTreeModel *model,
			 ...
			 gpointer      data)
{
	idol_playlist_emit_item_removed (data, iter);
	return FALSE;
}

gboolean
idol_playlist_clear (IdolPlaylist *playlist)
{
	GtkListStore *store;
...
static gboolean
idol_playlist_item_to_be_removed (IdolPlaylist *playlist,
				   GtkTreePath *path,
				   ClearComparisonFunc func)
{
	GList *ret;
...
static void
idol_playlist_clear_with_compare (IdolPlaylist *playlist,
				   ClearComparisonFunc func,
				   gconstpointer data)
{
	GtkTreeRowReference *ref;
	...
	if (func == NULL) {
		...
		gtk_tree_selection_selected_foreach (selection,
						     idol_playlist_foreach_selected,
						     (gpointer) playlist);
	} else {
		guint num_items, i;
	...
	if (playlist->priv->current_to_be_removed == FALSE &&
		...
		while (next != NULL) {
			if (idol_playlist_item_to_be_removed (playlist, item, func) == FALSE) {
	...
	while (playlist->priv->list != NULL) {
		...
		idol_playlist_emit_item_removed (playlist, &iter);
	...
	if (playlist->priv->current_to_be_removed != FALSE) {
		...
		g_signal_emit (G_OBJECT (playlist),
				idol_playlist_table_signals[CURRENT_REMOVED],
				0, NULL);
	} else {
		if (ref != NULL) {
...
static gboolean
idol_playlist_compare_with_mount (IdolPlaylist *playlist, GtkTreeIter *iter, gconstpointer data)
{
	...
	gtk_tree_model_get (playlist->priv->model, iter,
			    URI_COL, &mrl, -1);
	mount = idol_get_mount_for_media (mrl);
	g_free (mrl);
...
void
idol_playlist_clear_with_g_mount (IdolPlaylist *playlist,
				   GMount *mount)
{
	idol_playlist_clear_with_compare (playlist,
					   (ClearComparisonFunc) idol_playlist_compare_with_mount,
					   mount);
}

char *
idol_playlist_get_current_mrl (IdolPlaylist *playlist, char **subtitle)
{
	GtkTreeIter iter;
	char *path;
	...
	g_return_val_if_fail (IDOL_IS_PLAYLIST (playlist), NULL);
...
char *
idol_playlist_get_current_title (IdolPlaylist *playlist, gboolean *custom)
{
	GtkTreeIter iter;
	char *path;
...
char *
idol_playlist_get_title (IdolPlaylist *playlist, guint title_index)
{
	GtkTreeIter iter;
	GtkTreePath *path;
	char *title;
...
gboolean
idol_playlist_has_previous_mrl (IdolPlaylist *playlist)
{
	GtkTreeIter iter;
	...
	if (playlist->priv->shuffle == FALSE)
	{
		...
		return idol_playlist_gtk_tree_model_iter_previous
...
gboolean
idol_playlist_has_next_mrl (IdolPlaylist *playlist)
{
	GtkTreeIter iter;
...
gboolean
idol_playlist_set_title (IdolPlaylist *playlist, const char *title, gboolean force)
{
	GtkListStore *store;
	GtkTreeIter iter;
	gboolean custom_title;
	...
	g_signal_emit (playlist,
		       idol_playlist_table_signals[ACTIVE_NAME_CHANGED], 0);

	return TRUE;
}

gboolean
idol_playlist_set_playing (IdolPlaylist *playlist, IdolPlaylistStatus state)
{
	GtkListStore *store;
	GtkTreeIter iter;
	GtkTreePath *path;
...
IdolPlaylistStatus
idol_playlist_get_playing (IdolPlaylist *playlist)
{
	GtkTreeIter iter;
	IdolPlaylistStatus status;

	g_return_val_if_fail (IDOL_IS_PLAYLIST (playlist), IDOL_PLAYLIST_STATUS_NONE);

	if (gtk_tree_model_get_iter (playlist->priv->model, &iter, playlist->priv->current) == FALSE)
		return IDOL_PLAYLIST_STATUS_NONE;
...
void
idol_playlist_set_previous (IdolPlaylist *playlist)
{
	GtkTreeIter iter;

	g_return_if_fail (IDOL_IS_PLAYLIST (playlist));

	if (idol_playlist_has_previous_mrl (playlist) == FALSE)
		return;
	...
	if (playlist->priv->shuffle == FALSE)
	{
		...
		if (strcmp (path, "0") == 0)
		{
			idol_playlist_set_at_end (playlist);
			g_free (path);
			return;
		}
		g_free (path);
		...
		idol_playlist_gtk_tree_model_iter_previous
			(playlist->priv->model, &iter);
		gtk_tree_path_free (playlist->priv->current);
...
void
idol_playlist_set_next (IdolPlaylist *playlist)
{
	GtkTreeIter iter;

	g_return_if_fail (IDOL_IS_PLAYLIST (playlist));

	if (idol_playlist_has_next_mrl (playlist) == FALSE) {
		idol_playlist_set_at_start (playlist);
		return;
	}
...
gboolean
idol_playlist_get_repeat (IdolPlaylist *playlist)
{
	g_return_val_if_fail (IDOL_IS_PLAYLIST (playlist), FALSE);
...
void
idol_playlist_set_repeat (IdolPlaylist *playlist, gboolean repeat)
{
	g_return_if_fail (IDOL_IS_PLAYLIST (playlist));
...
gboolean
idol_playlist_get_shuffle (IdolPlaylist *playlist)
{
	g_return_val_if_fail (IDOL_IS_PLAYLIST (playlist), FALSE);
...
void
idol_playlist_set_shuffle (IdolPlaylist *playlist, gboolean shuffle)
{
	g_return_if_fail (IDOL_IS_PLAYLIST (playlist));
...
void
idol_playlist_set_at_start (IdolPlaylist *playlist)
{
	g_return_if_fail (IDOL_IS_PLAYLIST (playlist));
...
void
idol_playlist_set_at_end (IdolPlaylist *playlist)
{
	int indice;
...
int
idol_playlist_get_current (IdolPlaylist *playlist)
{
	char *path;
	double current_index;

	g_return_val_if_fail (IDOL_IS_PLAYLIST (playlist), -1);
...
int
idol_playlist_get_last (IdolPlaylist *playlist)
{
	guint len = PL_LEN;
...
void
idol_playlist_set_current (IdolPlaylist *playlist, guint current_index)
{
	g_return_if_fail (IDOL_IS_PLAYLIST (playlist));
...
static void
idol_playlist_class_init (IdolPlaylistClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	g_type_class_add_private (klass, sizeof (IdolPlaylistPrivate));

	object_class->dispose = idol_playlist_dispose;
	object_class->finalize = idol_playlist_finalize;
	...
	idol_playlist_table_signals[CHANGED] =
		g_signal_new ("changed",
				...
				G_SIGNAL_RUN_LAST,
				G_STRUCT_OFFSET (IdolPlaylistClass, changed),
				NULL, NULL,
				g_cclosure_marshal_VOID__VOID,
				G_TYPE_NONE, 0);
	idol_playlist_table_signals[ITEM_ACTIVATED] =
		g_signal_new ("item-activated",
				G_TYPE_FROM_CLASS (klass),
				G_SIGNAL_RUN_LAST,
				G_STRUCT_OFFSET (IdolPlaylistClass, item_activated),
				NULL, NULL,
				g_cclosure_marshal_VOID__VOID,
				G_TYPE_NONE, 0);
	idol_playlist_table_signals[ACTIVE_NAME_CHANGED] =
		g_signal_new ("active-name-changed",
				G_TYPE_FROM_CLASS (klass),
				G_SIGNAL_RUN_LAST,
				G_STRUCT_OFFSET (IdolPlaylistClass, active_name_changed),
				NULL, NULL,
				g_cclosure_marshal_VOID__VOID,
				G_TYPE_NONE, 0);
	idol_playlist_table_signals[CURRENT_REMOVED] =
		g_signal_new ("current-removed",
				G_TYPE_FROM_CLASS (klass),
				G_SIGNAL_RUN_LAST,
				G_STRUCT_OFFSET (IdolPlaylistClass,
						 current_removed),
				NULL, NULL,
				g_cclosure_marshal_VOID__VOID,
				G_TYPE_NONE, 0);
	idol_playlist_table_signals[REPEAT_TOGGLED] =
	...
	idol_playlist_table_signals[SHUFFLE_TOGGLED] =
		g_signal_new ("shuffle-toggled",
				...
				g_cclosure_marshal_VOID__BOOLEAN,
				G_TYPE_NONE, 1, G_TYPE_BOOLEAN);
	idol_playlist_table_signals[SUBTITLE_CHANGED] =
	...
	idol_playlist_table_signals[ITEM_ADDED] =
		g_signal_new ("item-added",
				...
				G_STRUCT_OFFSET (IdolPlaylistClass,
					item_added),
				NULL, NULL,
				idolplaylist_marshal_VOID__STRING_STRING,
...
static gboolean
idol_playlist_foreach_cb (GtkTreeModel *model,
			   GtkTreePath  *path,
			   GtkTreeIter  *iter,
			   gpointer      data)
{
...
void
idol_playlist_foreach (IdolPlaylist            *playlist,
			IdolPlaylistForeachFunc  callback,
			gpointer                  user_data)
{
