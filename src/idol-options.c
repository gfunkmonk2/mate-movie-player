...
IdolCmdLineOptions optionstate;	/* Decoded command line options */
...
void
idol_options_process_late (Idol *idol, const IdolCmdLineOptions *options)
{
	if (options->togglecontrols) 
		idol_action_toggle_controls (idol);

	/* Handle --playlist-idx */
	idol->index = options->playlistidx;

	/* Handle --seek */
	idol->seek_to_start = options->seek;
}

void
idol_options_register_remote_commands (Idol *idol)
{
	GEnumClass *klass;
	guint i;

	klass = (GEnumClass *) g_type_class_ref (IDOL_TYPE_REMOTE_COMMAND);
	for (i = IDOL_REMOTE_COMMAND_UNKNOWN + 1; i < klass->n_values; i++) {
		GEnumValue *val;
		val = g_enum_get_value (klass, i);
		unique_app_add_command (idol->app, val->value_name, i);
	}
	g_type_class_unref (klass);
}

void
idol_options_process_early (Idol *idol, const IdolCmdLineOptions* options)
{
	if (options->quit) {
	...
	mateconf_client_set_bool (idol->gc, MATECONF_PREFIX"/debug",
			       options->debug, NULL);
}

void
idol_options_process_for_server (UniqueApp *app,
				  const IdolCmdLineOptions* options)
{
	...
	commands = NULL;
	default_action = IDOL_REMOTE_COMMAND_REPLACE;

	/* Are we quitting ? */
	if (options->quit) {
		unique_app_send_message (app, IDOL_REMOTE_COMMAND_QUIT, NULL);
		return;
	}
	...
	if (options->replace && options->enqueue) {
		...
		default_action = IDOL_REMOTE_COMMAND_ENQUEUE;
	}
	...
	for (i = 0; options->filenames && options->filenames[i] != NULL; i++) {
		...
		full_path = idol_create_full_path (options->filenames[i]);
	...
	if (commands == NULL && options->filenames == NULL) {
		unique_app_send_message (app, IDOL_REMOTE_COMMAND_SHOW, NULL);
		return;
	}
