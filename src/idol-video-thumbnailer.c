...
add_holes_to_pixbuf_small (GdkPixbuf *pixbuf, int width, int height)
{
	...
	filename = g_build_filename (DATADIR, "idol", "filmholes.png", NULL);
...
add_holes_to_pixbuf_large (GdkPixbuf *pixbuf, int size)
{
	...
	filename = g_build_filename (DATADIR, "idol",
	     ...
	     const char *video_path, int size, gboolean is_still)
{
	...
	if (ret == FALSE) {
		if (err != NULL) {
			g_print ("idol-video-thumbnailer couldn't write the thumbnail '%s' for video '%s': %s\n", path, video_path, err->message);
			g_error_free (err);
		} else {
			g_print ("idol-video-thumbnailer couldn't write the thumbnail '%s' for video '%s'\n", path, video_path);
		}
			   ...
			   const char *output) 
{
	...
	for (current = 0; current < G_N_ELEMENTS(frame_locations); current++)
	{
		...
		{
			g_print ("idol-video-thumbnailer: '%s' isn't thumbnailable\n"
				 "Reason: %s\n",
		      ...
		      gint64 milliseconds)
{
	...
	if (bacon_video_widget_seek_time (bvw, milliseconds, TRUE, &err) == FALSE) {
		g_print ("idol-video-thumbnailer: could not seek to %d milliseconds in '%s'\n"
			 "Reason: %s\n",
...
create_gallery (BaconVideoWidget *bvw, const char *input, const char *output)
{
	...
	duration_text = idol_time_to_string (stream_length);
	filename = NULL;
	if (strstr (input, "://")) {
		char *local;
	...
	for (pos = screenshot_interval; pos <= stream_length; pos += screenshot_interval) {
		...
		timestamp_text = idol_time_to_string (pos);
...
int main (int argc, char *argv[])
{
	...
	if (err != NULL) {
		g_print ("idol-video-thumbnailer couldn't create the video "
				"widget.\nReason: %s.\n", err->message);
		g_error_free (err);
		exit (1);
	}
	...
	if (pixbuf == NULL) {
		g_print ("idol-video-thumbnailer couldn't get a picture from "
					"'%s'\n", input);
		exit (1);
	}
