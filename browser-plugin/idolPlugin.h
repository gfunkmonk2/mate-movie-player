...
#include "idolNPObjectWrapper.h"
#include "idolNPVariantWrapper.h"

#include "idol-plugin-viewer-constants.h"

#define IDOL_NARROWSPACE_VERSION   "7.6.6"
#define IDOL_MULLY_VERSION         "1.4.0.233"
#define IDOL_CONE_VERSION          "0.8.6"
#define IDOL_GMP_VERSION_BUILD     "11.0.0.1024"
...
typedef enum {
	IDOL_QUEUE_TYPE_SET_VOLUME,
	IDOL_QUEUE_TYPE_CLEAR_PLAYLIST,
	IDOL_QUEUE_TYPE_ADD_ITEM,
	IDOL_QUEUE_TYPE_SET_BOOLEAN,
	IDOL_QUEUE_TYPE_SET_STRING
} IdolQueueCommandType;

typedef struct {
	IdolQueueCommandType type;
	union {
		float volume;
		struct {
			char *uri;
...
class idolBasicPlayer;
class idolConePlayer;
class idolGMPControls;
class idolGMPError;
class idolGMPPlayer;
class idolGMPSettings;
class idolMullYPlayer;
class idolNarrowSpacePlayer;
...
class idolPlugin {
  public:
    ...
    static void PluginMimeTypes (const idolPluginMimeEntry **, uint32_t *);
  ...
  private:
    ...
    idolNPObjectWrapper mPluginElement;
  ...
  private:
    ...
    void LaunchIdol (const char* aURL,
		      uint32_t aTimestamp);
  ...
  private:
    ...
    idolNPObjectWrapper mNPObjects[eLastNPObject];
  ...
  public:
    ...
    void QueueCommand (IdolQueueCommand *cmd);
    ...
    IdolStates State () const { return mState; }
