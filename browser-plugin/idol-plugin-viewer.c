...
#include "idol-glow-button.h"
#include "video-utils.h"

#include "idol-plugin-viewer-constants.h"
#include "idol-plugin-viewer-options.h"
#include "marshal.h"

GtkWidget *idol_statusbar_create (void);
GtkWidget *idol_volume_create (void);
GtkWidget *idol_pp_create (void);

/* Private function in idol-pl-parser, not for use
 * by anyone but us */
char * idol_pl_parser_resolve_uri (GFile *base_gfile, const char *relative_uri);
...
typedef enum {
	IDOL_PLUGIN_TYPE_GMP,
	IDOL_PLUGIN_TYPE_NARROWSPACE,
	IDOL_PLUGIN_TYPE_MULLY,
	IDOL_PLUGIN_TYPE_CONE,
	IDOL_PLUGIN_TYPE_LAST
} IdolPluginType;

#define IDOL_TYPE_EMBEDDED (idol_embedded_get_type ())
#define IDOL_EMBEDDED(o)          (G_TYPE_CHECK_INSTANCE_CAST ((o), IDOL_TYPE_EMBEDDED, IdolEmbedded))
#define IDOL_EMBEDDED_CLASS(k)    (G_TYPE_CHECK_CLASS_CAST((k), IDOL_TYPE_EMBEDDED, IdolEmbeddedClass))
#define IDOL_IS_EMBEDDED(o)       (G_TYPE_CHECK_INSTANCE_TYPE ((o), IDOL_TYPE_EMBEDDED))
#define IDOL_IS_EMBEDDED_CLASS(k) (G_TYPE_CHECK_CLASS_TYPE ((k), IDOL_TYPE_EMBEDDED))
#define IDOL_EMBEDDED_GET_CLASS(o)(G_TYPE_INSTANCE_GET_CLASS ((o), IDOL_TYPE_EMBEDDED, IdolEmbeddedClass))

typedef GObjectClass IdolEmbeddedClass;
...
typedef struct _IdolEmbedded {
	...
	IdolStatusbar *statusbar;
	IdolScrsaver *scrsaver;
	int width, height;
        char *user_agent;
	const char *mimetype;
        char *referrer_uri;
	...
	/* Playlist, a GList of IdolPlItem */
...
GType idol_embedded_get_type (void);

#define IDOL_EMBEDDED_ERROR_QUARK (g_quark_from_static_string ("IdolEmbeddedErrorQuark"))

enum
{
	IDOL_EMBEDDED_UNKNOWN_PLUGIN_TYPE,
	IDOL_EMBEDDED_SETWINDOW_UNSUPPORTED_CONTROLS,
	IDOL_EMBEDDED_SETWINDOW_HAVE_WINDOW,
	IDOL_EMBEDDED_SETWINDOW_INVALID_XID,
	IDOL_EMBEDDED_NO_URI,
	IDOL_EMBEDDED_OPEN_FAILED,
	IDOL_EMBEDDED_UNKNOWN_COMMAND
};

G_DEFINE_TYPE (IdolEmbedded, idol_embedded, G_TYPE_OBJECT);
static void idol_embedded_init (IdolEmbedded *emb) { }

static gboolean idol_embedded_do_command (IdolEmbedded *emb, const char *command, GError **err);
static gboolean idol_embedded_push_parser (gpointer data);
static gboolean idol_embedded_play (IdolEmbedded *embedded, GError **error);
static void idol_embedded_set_logo_by_name (IdolEmbedded *embedded, const char *name);

static void idol_embedded_update_menu (IdolEmbedded *emb);
static void on_open1_activate (GtkButton *button, IdolEmbedded *emb);
static void idol_embedded_toggle_fullscreen (IdolEmbedded *emb);

void on_about1_activate (GtkButton *button, IdolEmbedded *emb);
void on_preferences1_activate (GtkButton *button, IdolEmbedded *emb);
void on_copy_location1_activate (GtkButton *button, IdolEmbedded *emb);
void on_fullscreen1_activate (GtkMenuItem *menuitem, IdolEmbedded *emb);

static void update_fill (IdolEmbedded *emb, gdouble level);
...
static void
idol_embedded_finalize (GObject *object)
{
	IdolEmbedded *embedded = IDOL_EMBEDDED (object);
	...
	G_OBJECT_CLASS (idol_embedded_parent_class)->finalize (object);
}

static void idol_embedded_class_init (IdolEmbeddedClass *klass)
{
	GType param_types[3];
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	object_class->finalize = idol_embedded_finalize;
	...
	signals[BUTTON_PRESS] =
		g_signal_newv ("button-press",
				...
				idolpluginviewer_marshal_VOID__UINT_UINT,
	...
	signals[SIGNAL_TICK] =
		g_signal_newv ("tick",
				...
				NULL /* class closure */,
				NULL /* accu */, NULL /* accu data */,
				idolpluginviewer_marshal_VOID__UINT_UINT_STRING,
	...
	signals[PROPERTY_CHANGE] =
		g_signal_new ("property-change",
			      ...
			      0 /* class offset */,
			      NULL /* accu */, NULL /* accu data */,
			      idolpluginviewer_marshal_VOID__STRING_BOXED,
			      G_TYPE_NONE, 2, G_TYPE_STRING, G_TYPE_VALUE);
}

static void
idol_embedded_exit (IdolEmbedded *emb)
{
...
static void
idol_embedded_error_and_exit (char *title, char *reason, IdolEmbedded *emb)
{
	...
	idol_embedded_exit (emb);
}

static void
idol_embedded_volume_changed (IdolEmbedded *emb, double volume)
{
	GValue value = { 0, };
	...
	g_signal_emit (emb, signals[PROPERTY_CHANGE], 0,
		       IDOL_PROPERTY_VOLUME,
		       &value);
}

static void
idol_embedded_set_error (IdolEmbedded *emb,
			  int code,
			  char *secondary)
{
	emb->error = g_error_new_literal (IDOL_EMBEDDED_ERROR_QUARK,
	                                  code,
	                                  secondary);
	g_message ("idol_embedded_set_error: '%s'", secondary);
}

static gboolean
idol_embedded_set_error_logo (IdolEmbedded *embedded,
			       GError *error)
{
	g_message ("idol_embedded_set_error_logo called by browser plugin");
	idol_embedded_set_logo_by_name (embedded, "image-missing");
	return TRUE;
}


static void
idol_embedded_set_state (IdolEmbedded *emb, IdolStates state)
{
	GtkWidget *image;
	const gchar *id;
	...
	g_message ("Viewer state: %s", idol_states[state]);

	image = gtk_button_get_image (GTK_BUTTON (emb->pp_button));

	switch (state) {
	case IDOL_STATE_STOPPED:
		id = GTK_STOCK_MEDIA_PLAY;
		idol_statusbar_set_text (emb->statusbar, _("Stopped"));
		idol_statusbar_set_time_and_length (emb->statusbar, 0, 0);
		idol_time_label_set_time 
			(IDOL_TIME_LABEL (emb->fs->time_label), 0, 0);
		...
		break;
	case IDOL_STATE_PAUSED:
		id = GTK_STOCK_MEDIA_PLAY;
		idol_statusbar_set_text (emb->statusbar, _("Paused"));
		break;
	case IDOL_STATE_PLAYING:
		id = GTK_STOCK_MEDIA_PAUSE;
		idol_statusbar_set_text (emb->statusbar, _("Playing"));
		if (emb->href_uri == NULL && emb->hidden == FALSE) {
			gdk_window_set_cursor
				(gtk_widget_get_window (GTK_WIDGET (emb->bvw)),
				 NULL);
	...
	if (emb->scrsaver != NULL)
		idol_scrsaver_set_state (emb->scrsaver, (state == IDOL_STATE_PLAYING) ? FALSE : TRUE);
...
static void
idol_embedded_set_logo_by_name (IdolEmbedded *embedded,
				 const char *name)
{
	idol_embedded_set_state (embedded, IDOL_STATE_STOPPED);
...
static void
idol_embedded_set_pp_state (IdolEmbedded *emb, gboolean state)
{
...
static gboolean
idol_embedded_open_internal (IdolEmbedded *emb,
			      gboolean start_play,
			      GError **error)
{
	gboolean retval;
	...
	if (!uri) {
		g_set_error_literal (error,
                                     IDOL_EMBEDDED_ERROR_QUARK,
                                     IDOL_EMBEDDED_NO_URI,
                                     _("No URI to play"));
		//FIXME idol_embedded_set_error (emb, error); |error| may be null?

		return FALSE;
	}

	g_message ("idol_embedded_open_internal '%s' subtitle '%s' is-browser-stream %d start-play %d",
	...
	if (start_play)
		idol_embedded_play (emb, NULL);
	else
		idol_glow_button_set_glow (IDOL_GLOW_BUTTON (emb->pp_button), TRUE);

	idol_embedded_update_menu (emb);
	if (emb->href_uri != NULL)
		idol_fullscreen_set_title (emb->fs, emb->href_uri);
	else
		idol_fullscreen_set_title (emb->fs, emb->current_uri);

	return retval;
}

static gboolean
idol_embedded_play (IdolEmbedded *emb,
		     GError **error)
{
	GError *err = NULL;

	if (emb->current_uri == NULL) {
		idol_glow_button_set_glow (IDOL_GLOW_BUTTON (emb->pp_button), FALSE);
	...
	if (bacon_video_widget_play (emb->bvw, &err) != FALSE) {
		idol_embedded_set_state (emb, IDOL_STATE_PLAYING);
		idol_embedded_set_pp_state (emb, TRUE);
	} else {
...
static gboolean
idol_embedded_pause (IdolEmbedded *emb,
		      GError **error)
{
	idol_embedded_set_state (emb, IDOL_STATE_PAUSED);
...
static gboolean
idol_embedded_stop (IdolEmbedded *emb,
		     GError **error)
{
	idol_embedded_set_state (emb, IDOL_STATE_STOPPED);
	bacon_video_widget_stop (emb->bvw);

	g_signal_emit (emb, signals[SIGNAL_TICK], 0,
		       (guint32) 0,
		       (guint32) 0,
		       idol_states[emb->state]);

	return TRUE;
}

static gboolean
idol_embedded_do_command (IdolEmbedded *embedded,
			   const char *command,
			   GError **error)
{
	g_return_val_if_fail (command != NULL, FALSE);

	g_message ("idol_embedded_do_command: %s", command);

	if (strcmp (command, IDOL_COMMAND_PLAY) == 0) {
		return idol_embedded_play (embedded, error);
	}
	if (strcmp (command, IDOL_COMMAND_PAUSE) == 0) {
		return idol_embedded_pause (embedded, error);
	}
	if (strcmp (command, IDOL_COMMAND_STOP) == 0) {
		return idol_embedded_stop (embedded, error);
	}
...
static gboolean
idol_embedded_set_href (IdolEmbedded *embedded,
			 const char *href_uri,
			 const char *target,
			 GError *error)
{
	g_message ("idol_embedded_set_href %s (target: %s)",
		   href_uri, target);
...
static gboolean
idol_embedded_set_volume (IdolEmbedded *embedded,
			   gdouble volume,
			   GError *error)
{
	g_message ("idol_embedded_set_volume: %f", volume);
	bacon_video_widget_set_volume (embedded->bvw, volume);
	idol_embedded_volume_changed (embedded, volume);
	return TRUE;
}

static gboolean
idol_embedded_launch_player (IdolEmbedded *embedded,
 			      guint32 user_time)
{
	GList *uris = NULL;
	GdkScreen *screen;
	...
	if (embedded->type == IDOL_PLUGIN_TYPE_NARROWSPACE
		   && embedded->href_uri != NULL) {
...
static void
idol_embedded_set_uri (IdolEmbedded *emb,
		        const char *uri,
		        const char *base_uri,
		        const char *subtitle,
		        gboolean is_browser_stream)
{
	...
	emb->current_uri = idol_pl_parser_resolve_uri (base_gfile, uri);
	if (subtitle != NULL)
		emb->current_subtitle_uri = idol_pl_parser_resolve_uri (base_gfile, subtitle);
	else
	...
	if (uri != NULL)
		g_print ("idol_embedded_set_uri uri %s base %s => resolved %s (subtitle %s => resolved %s)\n",
...
static void
idol_embedded_update_title (IdolEmbedded *emb, const char *title)
{
	if (title == NULL)
		gtk_window_set_title (GTK_WINDOW (emb->fs_window), _("Idol Movie Player"));
	else
		gtk_window_set_title (GTK_WINDOW (emb->fs_window), title);
	idol_fullscreen_set_title (emb->fs, title);
}

static void
idol_pl_item_free (gpointer data, gpointer user_data)
{
	IdolPlItem *item = (IdolPlItem *) data;
	...
	g_slice_free (IdolPlItem, item);
}

static gboolean
idol_embedded_clear_playlist (IdolEmbedded *emb, GError *error)
{
	g_message ("idol_embedded_clear_playlist");

	g_list_foreach (emb->playlist, (GFunc) idol_pl_item_free, NULL);
	...
	idol_embedded_set_uri (emb, NULL, NULL, NULL, FALSE);

	bacon_video_widget_close (emb->bvw);
	update_fill (emb, -1.0);
	idol_embedded_update_title (emb, NULL);

	return TRUE;
}

static gboolean
idol_embedded_add_item (IdolEmbedded *embedded,
			 const char *base_uri,
			 const char *uri,
			 const char *title,
			 const char *subtitle,
			 GError *error)
{
	...
	g_message ("idol_embedded_add_item: %s (base: %s title: %s subtitle: %s)",
		   uri, base_uri, title, subtitle);

	item = g_slice_new0 (IdolPlItem);
	...
	if (embedded->current_uri == NULL) {
		...
		idol_embedded_set_uri (embedded,
					(const char *) uri,
					base_uri,
					subtitle,
					FALSE);
		idol_embedded_open_internal (embedded, FALSE, NULL /* FIXME */);
	}
...
static gboolean
idol_embedded_set_fullscreen (IdolEmbedded *emb,
			       ...
			       GError **error)
{
	...
	if (idol_fullscreen_is_fullscreen (emb->fs) == fullscreen_enabled)
		return TRUE;

	g_message ("idol_embedded_set_fullscreen: %d", fullscreen_enabled);
	...
	if (fullscreen_enabled == FALSE) {
		...
		idol_fullscreen_set_fullscreen (emb->fs, FALSE);
		...
		idol_fullscreen_set_fullscreen (emb->fs, TRUE);
...
static gboolean
idol_embedded_set_time (IdolEmbedded *emb,
			 guint64 time,
			 GError **error)
{
	g_message ("idol_embedded_set_time: %"G_GUINT64_FORMAT, time);
...
static gboolean
idol_embedded_open_uri (IdolEmbedded *emb,
			 const char *uri,
			 const char *base_uri,
			 GError **error)
{
	g_message ("idol_embedded_open_uri: uri %s base_uri: %s", uri, base_uri);

	idol_embedded_clear_playlist (emb, NULL);

	idol_embedded_set_uri (emb, uri, base_uri, NULL, FALSE);
	/* We can only have one item in the "playlist" when
	 * we open a particular URI like this */
	emb->num_items = 1;

	return idol_embedded_open_internal (emb, TRUE, error);
}

static gboolean
idol_embedded_setup_stream (IdolEmbedded *emb,
			     const char *uri,
			     const char *base_uri,
			     GError **error)
{
	g_message ("idol_embedded_setup_stream called: uri %s, base_uri: %s", uri, base_uri);

	idol_embedded_clear_playlist (emb, NULL);

	idol_embedded_set_uri (emb, uri, base_uri, NULL, TRUE);
	/* We can only have one item in the "playlist" when
...
static gboolean
idol_embedded_open_stream (IdolEmbedded *emb,
			    gint64 size,
			    GError **error)
{
	g_message ("idol_embedded_open_stream called: with size %"G_GINT64_FORMAT, size);
...
static gboolean
idol_embedded_close_stream (IdolEmbedded *emb,
			     GError *error)
{
	g_message ("idol_embedded_close_stream");
...
static gboolean
idol_embedded_open_playlist_item (IdolEmbedded *emb,
				   GList *item)
{
	IdolPlItem *plitem;
	gboolean eop;
	...
	plitem = (IdolPlItem *) item->data;

	idol_embedded_set_uri (emb,
	...
	idol_embedded_update_title (emb, plitem->title);
	if (idol_embedded_open_internal (emb, FALSE, NULL /* FIXME */)) {
...
static gboolean
idol_embedded_set_local_file (IdolEmbedded *emb,
			       const char *path,
			       const char *uri,
			       const char *base_uri,
			       GError **error)
{
	...
	idol_embedded_set_uri (emb, file_uri, base_uri, emb->current_subtitle_uri, FALSE);
	g_free (file_uri);
...
static gboolean
idol_embedded_set_local_cache (IdolEmbedded *emb,
				const char *path,
				GError **error)
{
	int fd;

	g_message ("idol_embedded_set_local_cache: %s", path);
...
static gboolean
idol_embedded_set_playlist (IdolEmbedded *emb,
			     const char *path,
			     const char *uri,
			     const char *base_uri,
			     GError **error)
{
	...
	fd = g_file_open_tmp ("idol-browser-plugin-playlist-XXXXXX",
			      &tmp_file,
			      &err);
	if (fd < 0) {
	...
	idol_embedded_set_uri (emb, file_uri, base_uri, NULL, FALSE);
	g_free (file_uri);

	/* Schedule parsing on idle */
	if (emb->parser_id == 0)
		emb->parser_id = g_idle_add (idol_embedded_push_parser,
					     emb);
...
static GAppInfo *
idol_embedded_get_app_for_uri (const char *uri)
{
	char *type;
	GAppInfo *info;
...
static void
idol_embedded_update_menu (IdolEmbedded *emb)
{
	...
	if (emb->mimetype && strcmp (emb->mimetype, "application/octet-stream") != 0) {
		...
		if (emb->stream_uri != NULL)
			...
			uri = emb->current_uri;
		emb->app = idol_embedded_get_app_for_uri (uri);
	}
...
static void
on_open1_activate (GtkButton *button, IdolEmbedded *emb)
{
	GTimeVal val;
	g_get_current_time (&val);
	idol_embedded_launch_player (emb, val.tv_sec);
	idol_embedded_stop (emb, NULL);
}

void
on_fullscreen1_activate (GtkMenuItem *menuitem, IdolEmbedded *emb)
{
	if (idol_fullscreen_is_fullscreen (emb->fs) == FALSE)
		idol_embedded_toggle_fullscreen (emb);
}

void
on_about1_activate (GtkButton *button, IdolEmbedded *emb)
{
	...
	description = g_strdup_printf (_("Browser Plugin using %s"),
				       backend_version);
	license = idol_interface_get_license ();

	emb->about = g_object_new (GTK_TYPE_ABOUT_DIALOG,
				   "program-name", _("Idol Browser Plugin"),
				   "version", VERSION,
	...
	idol_interface_set_transient_for (GTK_WINDOW (emb->about),
					   GTK_WINDOW (emb->window));
...
void
on_copy_location1_activate (GtkButton *button, IdolEmbedded *emb)
{
	GdkDisplay *display;
	GtkClipboard *clip;
	const char *uri;
...
void
on_preferences1_activate (GtkButton *button, IdolEmbedded *emb)
{
	/* TODO: */
}

static void
on_play_pause (GtkWidget *widget, IdolEmbedded *emb)
{
	if (emb->state == IDOL_STATE_PLAYING) {
		idol_embedded_pause (emb, NULL);
	} else {
		if (emb->current_uri == NULL) {
			idol_glow_button_set_glow (IDOL_GLOW_BUTTON (emb->pp_button), FALSE);
			g_signal_emit (emb, signals[BUTTON_PRESS], 0,
...
static void
on_popup_button_toggled (GtkToggleButton *button, IdolEmbedded *emb)
{
	GtkMenu *menu;
...
static void
on_got_redirect (GtkWidget *bvw, const char *mrl, IdolEmbedded *emb)
{
	char *new_uri = NULL;
	...
	/* FIXME: use idol_embedded_open_uri? */

	idol_embedded_set_uri (emb, new_uri ? new_uri : mrl , emb->base_uri /* FIXME? */, emb->current_subtitle_uri, FALSE);

	idol_embedded_set_state (emb, IDOL_STATE_STOPPED);

	idol_embedded_open_internal (emb, TRUE, NULL /* FIXME? */);

	g_free (new_uri);
}

static char *
idol_embedded_get_nice_name_for_stream (BaconVideoWidget *bvw)
{
...
static void
on_got_metadata (BaconVideoWidget *bvw, IdolEmbedded *emb)
{
	char *title;

	title = idol_embedded_get_nice_name_for_stream (bvw);
	if (title == NULL)
		return;

	idol_embedded_update_title (emb, title);
}

static void
idol_embedded_toggle_fullscreen (IdolEmbedded *emb)
{
	if (idol_fullscreen_is_fullscreen (emb->fs) != FALSE)
		idol_embedded_set_fullscreen (emb, FALSE, NULL);
	else
		idol_embedded_set_fullscreen (emb, TRUE, NULL);
}

static void
idol_embedded_on_fullscreen_exit (GtkWidget *widget, IdolEmbedded *emb)
{
	if (idol_fullscreen_is_fullscreen (emb->fs) != FALSE)
		idol_embedded_toggle_fullscreen (emb);
}
			     ...
			     IdolEmbedded *emb)
{
	...
	if (event->type == GDK_BUTTON_PRESS && event->button == 1 && state == 0 && emb->state == IDOL_STATE_STOPPED) {
		if (emb->error != NULL) {
			idol_interface_error (_("An error occurred"), emb->error->message, (GtkWindow *) (emb->window));
...
static void
on_eos_event (BaconVideoWidget *bvw, IdolEmbedded *emb)
{
	gboolean start_play;
	...
	if (emb->is_browser_stream) {
		...
		if (g_str_has_prefix (emb->current_uri, "file://") != FALSE) {
			...
			bacon_video_widget_close (emb->bvw);
			update_fill (emb, -1.0);
			idol_embedded_open_internal (emb, start_play, NULL /* FIXME? */);
		} else {
			/* FIXME: should find a way to enable playback of the stream again without re-requesting it */
			idol_embedded_set_pp_state (emb, FALSE);
		}
	/* FIXME? else if (emb->playing_nth_item == emb->playlist_num_items) ? */
		...
		idol_embedded_open_playlist_item (emb, emb->current->next);
	}
}

static gboolean
skip_unplayable_stream (IdolEmbedded *emb)
{
		...
		IdolEmbedded *emb)
{
	if (playback_stopped) {
		...
		if (emb->num_items > 1 && emb->current) {
			IdolPlItem *item = emb->current->data;
	...
	idol_embedded_set_error (emb, BVW_ERROR_GENERIC, message);
	idol_embedded_set_error_logo (emb, NULL);
}

static void
cb_vol (GtkWidget *val, gdouble value, IdolEmbedded *emb)
{
	bacon_video_widget_set_volume (emb->bvw, value);
	idol_embedded_volume_changed (emb, value);
}
...
on_tick (GtkWidget *bvw,
		...
		IdolEmbedded *emb)
{
	if (emb->state != IDOL_STATE_STOPPED) {
		...
		if (stream_length == 0) {
			idol_statusbar_set_time_and_length (emb->statusbar,
		...
		idol_time_label_set_time 
			(IDOL_TIME_LABEL (emb->fs->time_label),
...
static void
on_buffering (BaconVideoWidget *bvw, guint percentage, IdolEmbedded *emb)
{
//	g_message ("Buffering: %d %%", percentage);
}

static void
update_fill (IdolEmbedded *emb, gdouble level)
{
	if (level < 0.0) {
...
static void
on_download_buffering (BaconVideoWidget *bvw, gdouble level, IdolEmbedded *emb)
{
			   ...
			   IdolEmbedded *emb)
{
	...
	idol_embedded_volume_changed (emb, volume);
	g_signal_handlers_unblock_by_func (emb->volume, cb_vol, emb);
}

static gboolean
on_seek_start (GtkWidget *widget, GdkEventButton *event, IdolEmbedded *emb)
{
	...
	emb->seeking = TRUE;
	idol_time_label_set_seeking (IDOL_TIME_LABEL (emb->fs->time_label),
				      TRUE);
...
static gboolean
cb_on_seek (GtkWidget *widget, GdkEventButton *event, IdolEmbedded *emb)
{
...
static void
idol_embedded_action_volume_relative (IdolEmbedded *emb, double off_pct)
{
	double vol;
	...
	idol_embedded_volume_changed (emb, vol + off_pct);
}

static gboolean
idol_embedded_handle_key_press (IdolEmbedded *emb, GdkEventKey *event)
{
	switch (event->keyval) {
	case GDK_Escape:
		if (idol_fullscreen_is_fullscreen (emb->fs) != FALSE)
	...
	case GDK_space:
		...
		return TRUE;
	case GDK_Up:
		idol_embedded_action_volume_relative (emb, VOLUME_UP_OFFSET);
		return TRUE;
	case GDK_Down:
		idol_embedded_action_volume_relative (emb, VOLUME_DOWN_OFFSET);
		return TRUE;
	case GDK_Left:
	case GDK_Right:
		/* FIXME: */
...
static gboolean
idol_embedded_key_press_event (GtkWidget *widget, GdkEventKey *event,
				IdolEmbedded *emb)
{
	...
	if (event->state & GDK_CONTROL_MASK)
	{
		...
		case GDK_Left:
		case GDK_Right:
			return idol_embedded_handle_key_press (emb, event);
		}
	}
...
static gboolean
idol_embedded_construct (IdolEmbedded *emb,
			  GdkNativeWindow xid,
			  int width,
			  int height)
{
	...
	emb->xml = idol_interface_load ("mozilla-viewer.ui", TRUE,
	...
	emb->fs = idol_fullscreen_new (GTK_WINDOW (emb->fs_window));
	idol_fullscreen_set_video_widget (emb->fs, emb->bvw);
	g_signal_connect (G_OBJECT (emb->fs->exit_button), "clicked",
			  G_CALLBACK (idol_embedded_on_fullscreen_exit), emb);
	...
	g_signal_connect (G_OBJECT(emb->fs_window), "key_press_event",
			G_CALLBACK (idol_embedded_key_press_event), emb);
	...
	/* popup */
	emb->menuxml = idol_interface_load ("mozilla-viewer.ui", TRUE,
					     GTK_WINDOW (emb->window), emb);
	...
	if (!emb->hidden && emb->autostart == FALSE) {
		...
		if (gtk_widget_get_direction (GTK_WIDGET (emb->bvw)) == GTK_TEXT_DIR_LTR)
			idol_embedded_set_logo_by_name (emb, "gtk-media-play-ltr");
		else
			idol_embedded_set_logo_by_name (emb, "gtk-media-play-rtl");
	} else {
		idol_embedded_set_logo_by_name (emb, "idol");
	}
...
static gboolean
idol_embedded_set_window (IdolEmbedded *embedded,
			   const char *controls,
			   guint window,
			   int width,
			   int height,
			   GError **error)
{
	...
	idol_embedded_construct (embedded, (GdkNativeWindow) window,
				  width, height);
...
static gboolean
idol_embedded_unset_window (IdolEmbedded *embedded,
			    guint window,
			    GError **error)
{
...
static void
entry_parsed (IdolPlParser *parser,
	     const char *uri,
	     GHashTable *metadata,
	     gpointer data)
{
	IdolEmbedded *emb = (IdolEmbedded *) data;
	IdolPlItem *item;
	int duration;
	...
	duration = idol_pl_parser_parse_duration (g_hash_table_lookup (metadata, IDOL_PL_PARSER_FIELD_DURATION), FALSE);
	if (duration == 0)
		return;

	item = g_slice_new0 (IdolPlItem);
	item->uri = g_strdup (uri);
	item->title = g_strdup (g_hash_table_lookup (metadata, IDOL_PL_PARSER_FIELD_TITLE));
	item->duration = duration;
	item->starttime = idol_pl_parser_parse_duration (g_hash_table_lookup (metadata, IDOL_PL_PARSER_FIELD_STARTTIME), FALSE);

	emb->playlist = g_list_prepend (emb->playlist, item);
}

static gboolean
idol_embedded_push_parser (gpointer data)
{
	IdolEmbedded *emb = (IdolEmbedded *) data;
	IdolPlParser *parser;
	...
	res = idol_pl_parser_parse_with_base (parser, emb->current_uri,
					       emb->base_uri, FALSE);
	g_object_unref (parser);

	/* Delete the temporary file created in
	 * idol_embedded_set_playlist */
	file = g_file_new_for_uri (emb->current_uri);
	g_file_delete (file, NULL, NULL);
	g_object_unref (file);

	if (res != IDOL_PL_PARSER_RESULT_SUCCESS) {
		//FIXME show a proper error message
		switch (res) {
		case IDOL_PL_PARSER_RESULT_UNHANDLED:
			g_print ("url '%s' unhandled\n", emb->current_uri);
			break;
		case IDOL_PL_PARSER_RESULT_ERROR:
			g_print ("error handling url '%s'\n", emb->current_uri);
			break;
		case IDOL_PL_PARSER_RESULT_IGNORED:
	...
	if (emb->playlist == NULL && res != IDOL_PL_PARSER_RESULT_SUCCESS) {
		g_message ("Couldn't parse playlist '%s'", emb->current_uri);
		idol_embedded_set_error (emb, BVW_ERROR_EMPTY_FILE,
	...
	idol_embedded_open_playlist_item (emb, emb->playlist);
...
static IdolPluginType arg_plugin_type = IDOL_PLUGIN_TYPE_LAST;
...
parse_plugin_type (const gchar *option_name,
	           ...
	           GError **error)
{
	const char types[IDOL_PLUGIN_TYPE_LAST][12] = {
		"gmp",
		"narrowspace",
		"mully",
		"cone"
	...
	for (type = 0; type < IDOL_PLUGIN_TYPE_LAST; ++type) {
...
{
	{ IDOL_OPTION_PLUGIN_TYPE, 0, 0, G_OPTION_ARG_CALLBACK, parse_plugin_type, NULL, NULL },
	{ IDOL_OPTION_USER_AGENT, 0, 0, G_OPTION_ARG_STRING, &arg_user_agent, NULL, NULL },
	{ IDOL_OPTION_MIMETYPE, 0, 0, G_OPTION_ARG_STRING, &arg_mime_type, NULL, NULL },
	{ IDOL_OPTION_CONTROLS_HIDDEN, 0, 0, G_OPTION_ARG_NONE, &arg_no_controls, NULL, NULL },
	{ IDOL_OPTION_STATUSBAR, 0, 0, G_OPTION_ARG_NONE, &arg_statusbar, NULL, NULL },
	{ IDOL_OPTION_HIDDEN, 0, 0, G_OPTION_ARG_NONE, &arg_hidden, NULL, NULL },
	{ IDOL_OPTION_PLAYLIST, 0, 0, G_OPTION_ARG_NONE, &arg_is_playlist, NULL, NULL },
	{ IDOL_OPTION_REPEAT, 0, 0, G_OPTION_ARG_NONE, &arg_repeat, NULL, NULL },
	{ IDOL_OPTION_NOAUTOSTART, 0, 0, G_OPTION_ARG_NONE, &arg_no_autostart, NULL, NULL },
	{ IDOL_OPTION_AUDIOONLY, 0, 0, G_OPTION_ARG_NONE, &arg_audioonly, NULL, NULL },
        { IDOL_OPTION_REFERRER, 0, 0, G_OPTION_ARG_STRING, &arg_referrer, NULL, NULL },
	{ G_OPTION_REMAINING, 0, 0, G_OPTION_ARG_FILENAME_ARRAY /* STRING? */, &arg_remaining, NULL },
	{ NULL }
};

#include "idol-plugin-viewer-interface.h"
...
int main (int argc, char **argv)
{
	...
	{
		gtk_init (&argc, &argv);
		idol_embedded_error_and_exit (_("Could not initialize the thread-safe libraries."), _("Verify your system installation. The Idol plugin will now exit."), NULL);
	}
...
#ifdef MATE_ENABLE_DEBUG
	{
		...
		env = g_getenv ("IDOL_EMBEDDED_GDB");
	...
	if (arg_plugin_type == IDOL_PLUGIN_TYPE_LAST) {
	...
	dbus_g_object_type_install_info (IDOL_TYPE_EMBEDDED,
					 &dbus_glib_idol_embedded_object_info);
	g_snprintf (svcname, sizeof (svcname),
		    IDOL_PLUGIN_VIEWER_NAME_TEMPLATE, getpid());
	...
	emb = g_object_new (IDOL_TYPE_EMBEDDED, NULL);

	emb->state = IDOL_STATE_INVALID;
	emb->width = -1;
	emb->height = -1;
	 ...
	 */
	if (emb->hidden) {
		idol_embedded_construct (emb, 0, -1, -1);
	}
