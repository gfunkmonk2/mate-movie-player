...
static NPError
idol_plugin_new_instance (NPMIMEType mimetype,
			   NPP instance,
			   uint16_t mode,
			   int16_t argc,
			   char *argn[],
			   ...
			   NPSavedData *savedData)
{
	...
	idolPlugin *plugin = new idolPlugin (instance);
	if (!plugin)
...
static NPError
idol_plugin_destroy_instance (NPP instance,
			       NPSavedData **save)
{
	if (!instance)
		return NPERR_INVALID_INSTANCE_ERROR;

	idolPlugin *plugin = reinterpret_cast<idolPlugin*> (instance->pdata);
	if (!plugin)
...
static NPError
idol_plugin_set_window (NPP instance,
			 NPWindow* window)
{
	if (!instance)
		return NPERR_INVALID_INSTANCE_ERROR;
...
static NPError
idol_plugin_new_stream (NPP instance,
			 NPMIMEType type,
			 NPStream* stream_ptr,
			 NPBool seekable,
			 uint16* stype)
...
static NPError
idol_plugin_destroy_stream (NPP instance,
			     NPStream* stream,
			     NPError reason)
{
	if (!instance) {
		g_debug ("idol_plugin_destroy_stream instance is NULL");
		/* FIXME? */
		return NPERR_NO_ERROR;
	}
...
static int32_t
idol_plugin_write_ready (NPP instance,
			  NPStream *stream)
{
	if (!instance)
		return -1;
...
static int32_t
idol_plugin_write (NPP instance,
		    NPStream *stream,
		    int32_t offset,
		    int32_t len,
		    void *buffer)
...
static void
idol_plugin_stream_as_file (NPP instance,
			     NPStream *stream,
			     const char* fname)
{
	if (!instance)
...
static void
idol_plugin_url_notify (NPP instance,
			 const char* url,
			 NPReason reason,
			 void* notifyData)
{
...
static void
idol_plugin_print (NPP instance,
...
static int16_t
idol_plugin_handle_event (NPP instance,
                           void* event)
{
...
static NPError
idol_plugin_get_value (NPP instance,
		        ...
		        void *value)
{
	...
	if (instance) {
                plugin = reinterpret_cast<idolPlugin*> (instance->pdata);
	}

	/* See NPPVariable in npapi.h */
	switch (variable) {
	case NPPVpluginNameString:
		*((char **)value) = idolPlugin::PluginDescription ();
		break;
	case NPPVpluginDescriptionString:
		*((char **)value) = idolPlugin::PluginLongDescription ();
		break;
...
static NPError
idol_plugin_set_value (NPP instance,
...
NP_GetValue (void *future,
	     ...
	     void *value)
{
	return idol_plugin_get_value (NULL, variable, value);
}

static gboolean
idol_plugin_mimetype_is_disabled (const char *mimetype,
				   GKeyFile *system,
				   GKeyFile *user)
{
	GError *error = NULL;
...
NP_GetMIMEDescription (void)
{
	...
	if (g_key_file_load_from_file (system,
				       SYSCONFDIR"/idol/browser-plugins.ini",
				       G_KEY_FILE_NONE,
				       NULL) == FALSE) {
	...
	const idolPluginMimeEntry *mimetypes;
	uint32_t count;
	idolPlugin::PluginMimeTypes (&mimetypes, &count);
	for (uint32_t i = 0; i < count; ++i) {
		char *desc;

		if (idol_plugin_mimetype_is_disabled (mimetypes[i].mimetype, system, user))
			continue;
	       ...
	       NPPluginFuncs *aPluginVTable)
{
	...
	aPluginVTable->newp           = NewNPP_NewProc (idol_plugin_new_instance);
	aPluginVTable->destroy        = NewNPP_DestroyProc (idol_plugin_destroy_instance);
	aPluginVTable->setwindow      = NewNPP_SetWindowProc (idol_plugin_set_window);
	aPluginVTable->newstream      = NewNPP_NewStreamProc (idol_plugin_new_stream);
	aPluginVTable->destroystream  = NewNPP_DestroyStreamProc (idol_plugin_destroy_stream);
	aPluginVTable->asfile         = NewNPP_StreamAsFileProc (idol_plugin_stream_as_file);
	aPluginVTable->writeready     = NewNPP_WriteReadyProc (idol_plugin_write_ready);
	aPluginVTable->write          = NewNPP_WriteProc (idol_plugin_write);
	aPluginVTable->print          = NewNPP_PrintProc (idol_plugin_print);
	aPluginVTable->event          = NewNPP_HandleEventProc (idol_plugin_handle_event);
	aPluginVTable->urlnotify      = NewNPP_URLNotifyProc (idol_plugin_url_notify);
	aPluginVTable->javaClass      = NULL; 
	aPluginVTable->getvalue       = NewNPP_GetValueProc (idol_plugin_get_value);
	aPluginVTable->setvalue       = NewNPP_SetValueProc (idol_plugin_set_value);

	g_debug ("NP_Initialize succeeded");

	return idolPlugin::Initialise ();
}
