...
#ifndef __IDOL_NPOBJECT_H__
#define __IDOL_NPOBJECT_H__
...
class idolPlugin;
class idolNPObject;
class idolNPClass_base;
template<class T> class idolNPClass;

class idolNPObject : public NPObject {
  public:
    idolNPObject (NPP);

    virtual ~idolNPObject ();

    void* operator new (size_t aSize) throw ();

  protected:
    friend class idolNPClass_base;
  ...
  protected:
    ...
    idolPlugin* Plugin () const { assert (IsValid ()); return mPlugin; }
  ...
  private:
    ...
    idolNPClass_base* GetClass() const { return static_cast<idolNPClass_base*>(_class); }
};

/* Helper macros */
#define IDOL_LOG_CTOR() g_debug ("%s [%p]", __func__, (void*) this)
#define IDOL_LOG_DTOR() g_debug ("%s [%p]", __func__, (void*) this)

#define IDOL_LOG_INVOKE(i, T) \
{\
...
#define IDOL_WARN_INVOKE_UNIMPLEMENTED(i, T) \
{\
...
#define IDOL_WARN_1_INVOKE_UNIMPLEMENTED(i, T) \
{\
...
#define IDOL_WARN_GETTER_UNIMPLEMENTED(i, T) \
{\
...
#define IDOL_WARN_1_GETTER_UNIMPLEMENTED(i, T) \
{\
  static bool logWarning;\
  if (!logWarning) {\
    g_warning ("WARNING: getter for property %s::%s is unimplemented", #T, propertyNames[i]);\
...
#define IDOL_WARN_SETTER_UNIMPLEMENTED(i, T) \
{\
  static bool logWarning[G_N_ELEMENTS (propertyNames)];\
  if (!logWarning[i]) {\
...
#define IDOL_WARN_1_SETTER_UNIMPLEMENTED(i, T) \
{\
  static bool logWarning;\
  if (!logWarning) {\
    g_warning ("WARNING: setter for property %s::%s is unimplemented", #T, propertyNames[i]);\
