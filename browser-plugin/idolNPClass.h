...
#ifndef __IDOL_NPCLASS_H__
#define __IDOL_NPCLASS_H__
...
class idolNPClass_base : public NPClass {
...
template <class T>
class idolNPClass : public idolNPClass_base {
  
  public:

    typedef idolNPClass<T> class_type;

    idolNPClass (const char *aPropertNames[],
                  uint32_t aPropertyCount,
                  const char *aMethodNames[],
                  uint32_t aMethodCount,
                  const char *aDefaultMethodName) :
      idolNPClass_base (aPropertNames, aPropertyCount,
...
#define IDOL_DEFINE_NPCLASS(T) \
class T##NPClass : public idolNPClass<T> {\
\
  public:\
\
    T##NPClass () throw ();\
...
#define IDOL_IMPLEMENT_NPCLASS(T, propertyNames, propertyNamesCount, methodNames, methodNamesCount, defaultMethodName) \
\
T##NPClass::class_type* T##NPClass::sInstance = 0; \
\
T##NPClass::T##NPClass () throw ()\
  : idolNPClass<T> (propertyNames,\
                     propertyNamesCount,\
                     methodNames,\
                     methodNamesCount,\
                     defaultMethodName)\
