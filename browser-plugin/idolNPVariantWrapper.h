...
#ifndef __IDOL_NPVARIANT_WRAPPER_H__
#define __IDOL_NPVARIANT_WRAPPER_H__
...
class idolNPVariantWrapper {

  public:

    idolNPVariantWrapper ()                  : mOwned (false) { VOID_TO_NPVARIANT (mVariant); }
    idolNPVariantWrapper (bool aValue)       : mOwned (false) { BOOLEAN_TO_NPVARIANT (aValue, mVariant); }
    idolNPVariantWrapper (uint32_t aValue)   : mOwned (false) { INT32_TO_NPVARIANT (aValue, mVariant);   }
    idolNPVariantWrapper (double aValue)     : mOwned (false) { DOUBLE_TO_NPVARIANT (aValue, mVariant);  }
    idolNPVariantWrapper (char *aValue)      : mOwned (false) { STRINGZ_TO_NPVARIANT (aValue, mVariant); }
//     idolNPVariantWrapper (NPString *aValue)  : mOwned (false) { STRINGN_TO_NPVARIANT (aValue, mVariant); }
    idolNPVariantWrapper (NPObject *aObject) : mOwned (false) { OBJECT_TO_NPVARIANT (aObject, mVariant); }

    idolNPVariantWrapper (const idolNPVariantWrapper& aOther) : mVariant (aOther.mVariant), mOwned (false) { }

    ~idolNPVariantWrapper () { Clear (); }
...
//     void SetString  (NPString *aValue)  { Clear (); STRINGN_TO_NPVARIANT (aValue, mVariant); }
    ...
    class GetterCopies {
      public:
       explicit GetterCopies (idolNPVariantWrapper& aTarget) : mTarget (aTarget) { }
        ~GetterCopies () { }

       operator NPVariant*() { return mTarget.StartAssignment (); }

      private:
        idolNPVariantWrapper& mTarget;
    };

  private:

    idolNPVariantWrapper& operator= (const idolNPVariantWrapper&); // not implemented
...
inline idolNPVariantWrapper::GetterCopies
getter_Copies (idolNPVariantWrapper &aTarget)
{
  return idolNPVariantWrapper::GetterCopies (aTarget);
}

#endif /* __IDOL_NPVARIANT_WRAPPER_H__ */
