...
static void idol_scrsaver_finalize   (GObject *object);

struct IdolScrsaverPrivate {
...
static void
screensaver_inhibit_dbus (IdolScrsaver *scr,
			  gboolean	 inhibit)
{
...
static void
screensaver_update_dbus_presence (IdolScrsaver *scr)
{
        IdolScrsaverPrivate *priv = scr->priv;
	gchar *name_owner;
...
static void
screensaver_init_dbus (IdolScrsaver *scr)
{
...
static void
screensaver_finalize_dbus (IdolScrsaver *scr)
{
...
#ifdef GDK_WINDOWING_X11
static void
screensaver_enable_x11 (IdolScrsaver *scr)
{
...
static void
screensaver_finalize_x11 (IdolScrsaver *scr)
{
	g_source_remove_by_user_data (scr);
}
#endif
...
static void
idol_scrsaver_get_property (GObject *object,
			     guint property_id,
			     GValue *value,
			     GParamSpec *pspec)
{
...
static void
idol_scrsaver_set_property (GObject *object,
			     guint property_id,
			     const GValue *value,
			     GParamSpec *pspec)
{
...
static void
idol_scrsaver_class_init (IdolScrsaverClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	g_type_class_add_private (klass, sizeof (IdolScrsaverPrivate));

	object_class->set_property = idol_scrsaver_set_property;
	object_class->get_property = idol_scrsaver_get_property;
	object_class->finalize = idol_scrsaver_finalize;
 ...
 *
 * Returns: a newly created #IdolScrsaver
 */
IdolScrsaver *
idol_scrsaver_new (void)
{
	return IDOL_SCRSAVER (g_object_new (IDOL_TYPE_SCRSAVER, NULL));
}

static void
idol_scrsaver_init (IdolScrsaver *scr)
{
...
void
idol_scrsaver_disable (IdolScrsaver *scr)
{
	g_return_if_fail (IDOL_SCRSAVER (scr));
...
void
idol_scrsaver_enable (IdolScrsaver *scr)
{
	g_return_if_fail (IDOL_SCRSAVER (scr));
...
void
idol_scrsaver_set_state (IdolScrsaver *scr, gboolean enable)
{
	g_return_if_fail (IDOL_SCRSAVER (scr));
	...
	if (enable == FALSE)
		idol_scrsaver_disable (scr);
	else
		idol_scrsaver_enable (scr);
}

static void
idol_scrsaver_finalize (GObject *object)
{
	IdolScrsaver *scr = IDOL_SCRSAVER (object);
...
#warning Unimplemented
	{}
        ...
        G_OBJECT_CLASS (idol_scrsaver_parent_class)->finalize (object);
}
